...
const goldenP5=JSON.parse(read('tests/vectors/golden-p5-corpus-v1.json'));
function selectPreviewPlanet(){
 const saved=globalThis.OFU;globalThis.OFU={};
 try{
  for(const file of ['src/kernel/sha256.js','src/kernel/p2-unicode.js','src/kernel/p2-canonical.js','src/domains/astronomy/p3-skeleton.js','src/domains/astronomy/p3-canonical.js','src/temporal/p4-temporal.js','src/domains/planetology/p5-canonical.js'])vm.runInThisContext(read(file),{filename:file});
  const O=globalThis.OFU,A=O.p3Astronomy,P5=O.p5Planetology,P=O.p2,masterSeed=Uint8Array.from({length:32},(_,i)=>i),ctx={masterSeed,semanticManifestHash:A.semanticManifestHash()};let chosen=null;
  outer:for(let y=0n;y<16n;y++)for(let x=0n;x<512n;x++){const base={galaxyX:48n,galaxyY:-50n,galaxyZ:-1n,sectorX:0n,sectorY:0n,sectorZ:0n,siteX:x,siteY:y,siteZ:0n},system=A.resolveSystem(ctx,base);if(system.status!=='PRESENT'||system.facts.planetCount===0n)continue;for(let slot=0n;slot<system.facts.planetCount;slot++){const key={...base,orbitSlot:slot},snapshot=A.planetaryInputSnapshot(ctx,key);if(snapshot.status==='PRESENT'&&snapshot.formation.bulkPriorClass==='TERRESTRIAL'&&snapshot.formation.baselineMassMilliEarth>=1000n&&snapshot.formation.baselineMassMilliEarth<=8000n){chosen={key,snapshot};break outer}}}
  if(!chosen)throw new Error('deterministic preview selector found no supported P5 planet');const physical=P5.realizePhysicalPlanet(ctx,P5.adaptP3PlanetaryInputSnapshot(chosen.snapshot));if(physical.status!=='SUPPORTED')throw new Error('deterministic preview planet is not P5-supported');const planetId=P.hex(physical.planetId);if(planetId!==goldenP5.shippedCrossRuntimeVector.planetId)throw new Error('preview selector drifted from shipped P5 cross-runtime golden');
  return Object.freeze({key:Object.freeze(Object.fromEntries(Object.entries(chosen.key).map(([k,v])=>[k,String(v)]))),planetId,selector:'P5_SHIPPED_CROSS_RUNTIME_GOLDEN_V1'});
