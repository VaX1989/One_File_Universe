...
let world=T.createLiveWorld({universeIdentity:universe,lineage,baseline:{kind:'synthetic'}});world=T.commit({world,command:eventInput(p4.events.find(x=>x.label==='e1'))}).world;world=T.commit({world,command:eventInput(p4.events.find(x=>x.label==='e3'))}).world;rejects(()=>T.commit({world,command:eventInput(p4.events.find(x=>x.label==='e2'))}),'P4 retroactive live event');rejections++;
let retryWorld=T.createLiveWorld({universeIdentity:universe,lineage,baseline:{kind:'synthetic'}});retryWorld=T.commit({world:retryWorld,command:eventInput(p4.events.find(x=>x.label==='e1'))}).world;const retry=T.commit({world:retryWorld,command:eventInput(p4.events.find(x=>x.label==='e1'))});assert(retry.duplicate===true,'P4 frontier retry duplicate');
...
const evidence={schema:'ofu-ind-conf-a-js-authority-evidence-v1',status:'PASS',source_commit:process.env.OFU_SOURCE_SHA||'LOCAL-UNPINNED',corpus_digest:manifest.corpus_digest,positive_checks:positive,rejection_checks:rejections,p2_protocol:p2.protocol,p4_protocol:p4.protocol,rejection_class_note:'Corpus classes are stable expectations; current JS exception strings are intentionally non-normative.'};
