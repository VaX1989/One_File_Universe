...
export function deterministicVariant(lineage, generation, policy) {
  ...
  const h = fnv1a32(`${lineage.id}|${generation}|${policy.salt ?? 'v2x16'}`);
  const span = maxStep * 2 + 1; const delta = maxStep === 0 ? 0 : (h % span) - maxStep;
  const variantTrait = Math.max(EVOLUTION_LIMITS.traitMin, Math.min(EVOLUTION_LIMITS.traitMax, trait + delta));
  return Object.freeze({ authority: LIFE_AUTHORITY, parentId: lineage.id, candidateId: `${lineage.id}:g${generation}:${h.toString(16).padStart(8, '0')}`, trait: variantTrait, delta: variantTrait - trait, hashHint32: h, identityClaim: 'RESEARCH_DETERMINISTIC_ADDRESS_HINT_NOT_CANONICAL_IDENTITY' });
}
...
export function selectionStep(lineages, environment, policy) {
  ...
  return lineages.map((lineage, index) => { assertRecord(lineage, `lineages[${index}]`); const population = nonNegativeInt(lineage.population, `lineages[${index}].population`, EVOLUTION_LIMITS.maxPopulation); const trait = int(lineage.trait, `lineages[${index}].trait`, EVOLUTION_LIMITS.traitMin, EVOLUTION_LIMITS.traitMax); const mismatch = Math.abs(trait - optimum); const viabilityPpm = mismatch >= width ? 0 : Math.floor(((width - mismatch) * 1_000_000) / width); const birthsRaw = mulDivFloor(population, birthPpm, 1_000_000, `births.${lineage.id}`); const births = mulDivFloor(birthsRaw, viabilityPpm, 1_000_000, `viableBirths.${lineage.id}`); const deaths = mulDivFloor(population, deathPpm, 1_000_000, `deaths.${lineage.id}`); const nextPopulation = Math.max(0, Math.min(EVOLUTION_LIMITS.maxPopulation, population + births - deaths)); return Object.freeze({ ...lineage, population: nextPopulation, viabilityPpm, births, deaths, extinct: nextPopulation === 0 }); });
}
