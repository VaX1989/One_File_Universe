# V2X-16 life / civilization / individuals / matter frontier

Authority: **RESEARCH_ONLY**. This subtree is a promotion laboratory and falsification package; it is not a shipping provider, canonical world truth, measured runtime evidence, P4 event authority, renderer authority, or persistence authority.

## Authorized V2 parallel base

- base SHA: `2977c11a0ac97eba8fd7b6b7df9c958ea1a2d9a7`
- base tree: `99e6b5ff6229d9c34d381e778c5689bf2367d256`
...
Research algorithms may be promoted only by the owning shipping lane or convergence owner. Promotion does not inherit scientific authority merely because an invariant is exact. In particular:

- evolution fingerprints are noncryptographic, noncanonical lookup/address hints;
- ecology resource/energy quanta remain abstract until unit/calibration contracts exist;
- civilization logistics/coordination models are scenario abstractions, not predictive economics or sociology;
- material reaction execution is a bounded declared scenario operator, not a physical rate law;
- transport conservation is exact only for the declared integer compartment model;
- microstructure evolution is representative, not a constitutive law;
- unsupported quantum regimes stay unsupported and pseudo-quantum electron-orbit visuals remain forbidden.

## Artifact reachability law

Research-only source must **not** masquerade as shipping reachability. A candidate becomes vertically complete only after its shipping owner proves:
...
1. V2X-08: length-prefixed, policy-bound deterministic evolution derivation and delimiter-alias falsification.
2. V2X-09: simultaneous same-route capacity contention oracle with explicit rejected-capacity/loss accounting; preserve richer shipping multi-hop routing.
3. V2X-10: anti-spoof identity/reconciliation principles are already present in the frozen V2 base; retain them as regression evidence rather than duplicate implementation.
4. V2X-12: balanced reaction execution and simultaneous conservative compartment transport as optional `MODEL_DERIVED_SIMULATION` capabilities, retaining explicit no-kinetics/no-equilibrium limitations.
