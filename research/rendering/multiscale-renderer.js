...
const SCENARIOS=Object.freeze({
 ASTRONOMICAL_OVERVIEW:{maxQueries:125,maxObjects:125},GALAXY_REGION:{maxQueries:65,maxObjects:64},SYSTEM_VIEW:{maxQueries:16,maxObjects:16},PLANET_ORBIT:{maxQueries:10,maxObjects:10},PLANET_APPROACH:{maxQueries:4,maxObjects:4},SURFACE_LOCAL:{maxQueries:2,maxObjects:64}
});
...
function plan(session,scenario,key){if(!SCENARIOS[scenario])throw new Error('unknown scenario');const out=[],q0=session.queries;
 ...
 else if(scenario==='GALAXY_REGION'){const g=resolve(session,'galaxy',key);if(g.result.status==='PRESENT')for(let z=0n;z<4n;z++)for(let y=0n;y<4n;y++)for(let x=0n;x<4n;x++){const s=resolve(session,'sector',{galaxyX:key.x,galaxyY:key.y,galaxyZ:key.z,x,y,z});if(s.result.status==='PRESENT')out.push(presentationObject(s.result,[x,y,z],1))}}
 else {const systemKey=key.systemKey||key;const sys=resolve(session,'system',systemKey);if(sys.result.status!=='PRESENT')return report(session,scenario,out,q0);
 ...
 return report(session,scenario,out,q0)}
function report(session,scenario,objects,q0=0){session.materialized=objects.length;const budget=SCENARIOS[scenario],queryCount=session.queries-q0;if(queryCount>budget.maxQueries)throw new Error('query budget exceeded');if(objects.length>budget.maxObjects)throw new Error('materialization budget exceeded');return Object.freeze({scenario,objects:Object.freeze(objects),queryCount,totalQueries:session.queries,materializedObjects:objects.length,cache:session.cache.stats(),budget})}
...
const GL_STATE=new WeakMap(),GL_LISTENERS=new WeakSet();
function initGL(canvas){let state=GL_STATE.get(canvas);if(state&&state.gl&&!state.gl.isContextLost())return state;let gl;try{gl=canvas.getContext('webgl2',{antialias:true,alpha:false})}catch{}if(!gl||gl.isContextLost())return null;if(!GL_LISTENERS.has(canvas)){canvas.addEventListener('webglcontextlost',e=>{e.preventDefault();GL_STATE.delete(canvas)});canvas.addEventListener('webglcontextrestored',()=>GL_STATE.delete(canvas));GL_LISTENERS.add(canvas)}const vs=shader(gl,gl.VERTEX_SHADER,'#version 300 es\nin vec3 p;void main(){gl_Position=vec4(p,1.0);gl_PointSize=7.0;}'),fs=shader(gl,gl.FRAGMENT_SHADER,'#version 300 es\nprecision mediump float;out vec4 c;void main(){c=vec4(0.72,0.86,1.0,1.0);}'),prog=gl.createProgram();gl.attachShader(prog,vs);gl.attachShader(prog,fs);gl.bindAttribLocation(prog,0,'p');gl.linkProgram(prog);if(!gl.getProgramParameter(prog,gl.LINK_STATUS))throw new Error(gl.getProgramInfoLog(prog)||'program link failed');gl.deleteShader(vs);gl.deleteShader(fs);state={gl,prog};GL_STATE.set(canvas,state);return state}
function renderWebGL2(canvas,scene,{camera=[0n,0n,0n]}={}){if(!canvas||typeof canvas.getContext!=='function')return{backend:'unavailable'};const state=initGL(canvas);if(!state)return{backend:'unavailable',contextLost:true};const {gl,prog}=state,coords=[];for(const o of scene.objects){if(!o.position)continue;const r=cameraRelative(o.position,camera),m=Math.max(1,Math.abs(r[0]),Math.abs(r[1]),Math.abs(r[2]));coords.push(r[0]/m*.7,r[1]/m*.7,r[2]/m*.7)}const buf=gl.createBuffer();gl.bindBuffer(gl.ARRAY_BUFFER,buf);gl.bufferData(gl.ARRAY_BUFFER,new Float32Array(coords),gl.STREAM_DRAW);gl.viewport(0,0,canvas.width,canvas.height);gl.clearColor(.01,.015,.03,1);gl.clear(gl.COLOR_BUFFER_BIT);gl.useProgram(prog);gl.enableVertexAttribArray(0);gl.vertexAttribPointer(0,3,gl.FLOAT,false,0,0);gl.drawArrays(gl.POINTS,0,coords.length/3);gl.deleteBuffer(buf);return{backend:'webgl2',drawCalls:1,renderedPoints:coords.length/3}}
function probe(canvas){const nav=typeof navigator!=='undefined'?navigator:null;let webgl2=false;if(canvas&&typeof canvas.getContext==='function'){try{const gl=canvas.getContext('webgl2');webgl2=!!gl&&!gl.isContextLost()}catch{}}return{webgl2,webgpu:!!(nav&&nav.gpu)}}
