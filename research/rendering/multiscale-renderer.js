...
function plan(session,scenario,key){if(!SCENARIOS[scenario])throw new Error('unknown scenario');const out=[],q0=session.queries;
 ...
 else if(scenario==='GALAXY_REGION'){const g=resolve(session,'galaxy',key);if(g.result.status==='PRESENT')for(let z=0n;z<4n;z++)for(let y=0n;y<4n;y++)for(let x=0n;x<4n;x++){const s=resolve(session,'sector',{galaxyX:key.x,galaxyY:key.y,galaxyZ:key.z,sectorX:x,sectorY:y,sectorZ:z});if(s.result.status==='PRESENT')out.push(presentationObject(s.result,[x,y,z],1))}}
