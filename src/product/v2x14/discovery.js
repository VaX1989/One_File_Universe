...
const VERSION='ofu-v2x14-discovery-3',AUTHORITY='PRESENTATION_ONLY';
...
function create(options={}){
 const product=options.product||O.v1LivingProduct,runtime=options.runtime||product?.runtime,yieldTask=options.yieldTask||(()=>new Promise(resolve=>root.setTimeout?root.setTimeout(resolve,0):Promise.resolve().then(resolve)));
 invariant(runtime&&typeof runtime.snapshot==='function','Living runtime required');invariant(typeof runtime.searchWorlds==='function','bounded searchWorlds API required');invariant(typeof runtime.enterKey==='function','public enterKey travel API required');invariant(typeof yieldTask==='function','yieldTask required');
 ...
 function snapshot(){return Object.freeze({schema:'ofu-v2x14-discovery-snapshot-3',version:VERSION,authority:AUTHORITY,goal,context,cursor,pages,worlds,running,lastError,lastOutcome,results:Object.freeze(rows.map(x=>Object.freeze({planetIdentity:x.planetIdentity,canonicalKey:x.canonicalKey,biologyState:x.biologyState??null,civilizationState:x.civilizationState??null,surfaceTemperatureMilliK:x.surfaceTemperatureMilliK??null}))),bounds:BOUNDS,globalEnumeration:false,navigationDelegatesTo:'v1LivingRuntime.enterKey',mutatesCanonicalSelectionDirectly:false,networkRequired:false,staleResultsAccepted:false,cancellableBetweenPages:true});}
