...
function derive(s={}){
 ...
 const current=crumbs.slice().reverse().find(x=>x.stage===stage),resolved=current||crumbs[crumbs.length-1];
...
function mount(options={}){
 ...
 function applyCurrent(node,value){let record=managedCurrent.get(node);if(!record){record={baseline:node.getAttribute('aria-current'),applied:null};managedCurrent.set(node,record);}if(value){node.setAttribute('aria-current',value);record.applied=value;return;}if(record.applied&&node.getAttribute('aria-current')===record.applied){if(record.baseline==null)node.removeAttribute('aria-current');else node.setAttribute('aria-current',record.baseline);}record.applied=null;}
 ...
 if(!viewport.hasAttribute('role'))setAttr(viewport,'role','region');setAttr(viewport,'aria-keyshortcuts',mergeTokens(viewport.getAttribute('aria-keyshortcuts'),SHORTCUTS));remember(viewport,'data-v2x14-stage');remember(viewport,'data-v2x14-authority');
