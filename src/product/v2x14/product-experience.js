...
function mount(options={}){
 ...
 let inputSyncScheduled=false;
 const scheduleInputSync=root.queueMicrotask?root.queueMicrotask.bind(root):(root.requestAnimationFrame||root.setTimeout||((fn)=>fn()));
 const inputListener=()=>{if(disposed||inputSyncScheduled)return;inputSyncScheduled=true;scheduleInputSync(()=>{inputSyncScheduled=false;if(!disposed)syncInput();});};for(const eventName of INPUT_EVENTS)viewport.addEventListener?.(eventName,inputListener,{capture:true,passive:true});sync();
