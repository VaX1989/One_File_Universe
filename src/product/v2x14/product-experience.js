...
function mount(options={}){
 ...
 const syncRail=p=>{if(!rail)return;const active=MICRO.has(p.stage)?'MICRO':p.stage;for(const b of rail.querySelectorAll('[data-living-scale]')){if(b.dataset.livingScale===active)b.setAttribute('aria-current','step');else b.removeAttribute('aria-current');}};
