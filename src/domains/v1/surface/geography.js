...
function knownPpm(v){return typeof v==='number'&&Number.isFinite(v)&&v>=0&&v<=PPM?Math.round(v):null}
...
function createModel(input={}){
 ...
 const planetClass=String(input.planetClass||'UNKNOWN').toUpperCase(),noSolidSurface=Boolean(input.noSolidSurface===true||['GAS_GIANT','ICE_GIANT','GIANT','NO_SOLID_SURFACE'].includes(planetClass));
 ...
 const sourceAuthority=String(input.sourceAuthority||'UNKNOWN_UNVERIFIED'),sourceProvenance=freeze(Array.isArray(input.sourceProvenance)?input.sourceProvenance.slice(0,16).map(String):[]);
 ...
 const modelAuthority=assumptionsUsed.length?'PRESENTATION_ONLY':AUTHORITY,presentationInputs=freeze({authority:modelAuthority,waterAreaPpm,iceAreaPpm,tectonicActivityPpm,volcanicActivityPpm,impactActivityPpm,erosionActivityPpm,aridityPpm});
 ...
 function descriptor(){return freeze({version:VERSION,authority:modelAuthority,planetIdentity,planetClass,sourceAuthority,sourceProvenance,inputs:scientificInputs,presentationAssumptions:freeze({authority:PRESENTATION_PRIORS.authority,claim:PRESENTATION_PRIORS.claim,fields:assumptionsUsed.slice().sort()}),bounds:{featureCount,maxFeatures:MAX_FEATURES,plates:plates.length,orogens:orogens.length,basins:basins.length,hotspots:hotspots.length,impacts:impacts.length,globalEnumeration:false},surfaceSupport:noSolidSurface?'NO_SOLID_SURFACE':'SUPPORTED_MODEL_PRESENTATION',claims:{canonicalElevation:false,measuredGeology:false,exactOceanArea:false,plateTectonicsCanonical:false}})}
 return Object.freeze({VERSION,AUTHORITY:modelAuthority,presentationInputs,planetIdentity,planetClass,noSolidSurface,scientificInputs,presentationAssumptions:freeze({authority:PRESENTATION_PRIORS.authority,claim:PRESENTATION_PRIORS.claim,fields:assumptionsUsed.slice().sort()}),waterAreaPpm:scientificInputs.waterAreaPpm,iceAreaPpm:scientificInputs.iceAreaPpm,tectonicActivityPpm:scientificInputs.tectonicActivityPpm,volcanicActivityPpm:scientificInputs.volcanicActivityPpm,impactActivityPpm:scientificInputs.impactActivityPpm,erosionActivityPpm:scientificInputs.erosionActivityPpm,aridityPpm:scientificInputs.aridityPpm,seaLevelCuePpm,features:freeze({plates,orogens,basins,hotspots,impacts}),sample,sampleUnit,rawAtUnit,slopeCue,descriptor});
}
O.v2x06Geography=Object.freeze({VERSION,AUTHORITY,PPM,MAX_FEATURES,PRESENTATION_PRIORS,createModel});
})(globalThis);
