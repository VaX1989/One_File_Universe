...
const AUTH=V.authority('v1.planetology.bulk-diagnostics','1.0.0',[SOURCE],
  'Deterministic spherical-body reference diagnostics derived from the existing modeled mass/radius inputs. The witness cross-checks reduced-order gravity, density and escape-velocity outputs without upgrading those inputs or the diagnostic to canonical physical truth.',[
    ...
    'Surface gravity and Newtonian escape velocity assume a spherical non-rotating body; mean density uses the spherical volume definition. Oblateness, rotation, pressure structure, relativity and EOS effects are not modeled.',
...
function sqrtRatioHalfEven(n,d){
  V.assert(typeof n==='bigint'&&typeof d==='bigint'&&n>=0n&&d>0n,'bulk diagnostics bigint square root');
  if(n===0n)return 0n;
  let lo=0n,hi=1n;
  while(hi*hi*d<=n)hi<<=1n;
  while(lo+1n<hi){const mid=(lo+hi)>>1n;if(mid*mid*d<=n)lo=mid;else hi=mid;}
  const midpointScaled=d*(4n*lo*lo+4n*lo+1n),valueScaled=4n*n;
  if(valueScaled<midpointScaled)return lo;
  if(valueScaled>midpointScaled)return lo+1n;
  return (lo&1n)===0n?lo:lo+1n;
}
...
function derive(planet){
  ...
  // One m/s equals one milli-km/s, so the integer Newtonian escape-speed witness uses the repository's existing milli-km/s field unit directly.
  const escapeVelocityMilliKms=safeNumber(sqrtRatioHalfEven(2n*G_NUM*massKg,G_DEN*radiusM),'escape velocity');
  const modelGravity=Number(c.gravity?.surfaceGravityMilliMs2),modelDensity=Number(c.composition?.bulkDensityKgM3),modelEscape=Number(c.gravity?.escapeVelocityMilliKms);
  ...
  const escapeAgreement=Number.isSafeInteger(modelEscape)?Object.freeze({existingReducedModelValue:modelEscape,referenceDiagnosticValue:escapeVelocityMilliKms,signedResidual:modelEscape-escapeVelocityMilliKms,absoluteDeltaPpm:ppmDelta(modelEscape,escapeVelocityMilliKms),empiricalErrorBarClaim:false}):null;
  return V.freezeDeep({version:VERSION,worldIdentity:planet.planetIdentity,supported:true,modelId:'rd16-spherical-bulk-diagnostic-1',
    ...
    diagnostics:Object.freeze({surfaceGravityMilliMs2,meanDensityKgM3,escapeVelocityMilliKms,geometry:'SPHERICAL_REFERENCE_BODY',gravityModel:'NEWTONIAN_SPHERICAL_REFERENCE',escapeModel:'NEWTONIAN_SPHERICAL_ESCAPE_REFERENCE',rotationCorrectionApplied:false,oblatenessCorrectionApplied:false,relativisticCorrectionApplied:false,eosApplied:false}),
    agreement:Object.freeze({surfaceGravity:gravityAgreement,meanDensity:densityAgreement,escapeVelocity:escapeAgreement,comparisonClass:'MODEL_TO_REFERENCE_DIAGNOSTIC_ONLY'}),
    ...
    researchLineage:Object.freeze({sourceBranch:SOURCE,harvestedConcept:'PURE_DERIVED_SPHERICAL_GRAVITY_DENSITY_ESCAPE_DIAGNOSTIC',researchAuthorityPromoted:false})});
}
...
O.v1BulkPhysicalDiagnostics=Object.freeze({VERSION,SOURCE,AUTHORITY:AUTH,PPM,derive,divHalfEven,sqrtRatioHalfEven,ppmDelta});
})(globalThis);
