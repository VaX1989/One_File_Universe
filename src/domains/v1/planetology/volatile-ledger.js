...
function normalize(values,total){
 const rows=KEYS.map((key,i)=>({key,value:BigInt(values[i])})),T=BigInt(total),work=[];let used=0n;
 ...
 let left=BigInt(PPM)-used;const rank=[...work].sort((a,b)=>a.rem===b.rem?a.i-b.i:a.rem>b.rem?-1:1);
...
function witness(planet){
 ...
 const initial=BigInt(a.initialInventoryUnits),sum=values.reduce((n,x)=>n+BigInt(x),0n),residual=initial-sum,shares=normalize(values,a.initialInventoryUnits),shareClosure=shares.reduce((n,x)=>n+x.sharePpm,0);
 V.assert(shareClosure===PPM,'volatile ledger share closure');
 return V.freezeDeep({version:VERSION,worldIdentity:planet.planetIdentity,supported:true,modelUnit:'NORMALIZED_VOLATILE_INVENTORY_UNIT',initialInventoryUnits:a.initialInventoryUnits,
   ...
   closure:Object.freeze({sumUnits:Number(sum),residualUnits:Number(residual),exactInternalClosure:residual===0n,sourceConservedFlag:a.conserved===true}),shares,shareClosurePpm:shareClosure,
