...
export function projectIndividuals(people) {
  ...
  return Object.freeze({ materializedCount: people.length, roleCounts: Object.freeze(Object.fromEntries([...roles.entries()].sort(([a], [b]) => a.localeCompare(b)))) });
}

export function reconcileIndividuals({ aggregate, people, startOrdinal = 0 } = {}) {
  if (!aggregate || !Number.isSafeInteger(aggregate.population)) throw new TypeError('aggregate.population is required');
...
export function reconcileHouseholds({ people = [], households = [] } = {}) {
  ...
  for (const household of households) {
    ...
    const expectedMembers = (expected.get(household.id) || []).slice().sort();
    const actualMembers = [...household.activeMemberIds].sort();
