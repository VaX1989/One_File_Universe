...
function heatShares(interior){
  const h=interior?.heat||{},raw=[h.primordialPpm,h.radiogenicPpm,h.tidalPpm];
...
function interiorState(planet,regime){
  ...
  const h=x.interior?.heat||{},age=x.inputs.ageMyr,heat=h.heatIndexPpm,conv=x.interior.convectiveVigorPpm,volc=x.interior.volcanismPpm,dynamo=x.interior.dynamoPpm;
  ...
  const layers=[x.interior.coreFractionPpm,x.interior.mantleFractionPpm,x.interior.crustFractionPpm],differentiation=x.interior.differentiationPpm;
  ...
  const volatileMobilityPpm=C.clamp(x.composition.icePpm+x.composition.lightVolatilePpm,0,PPM);
  const tectonicPotentialPpm=giant?null:C.clamp(Math.round(conv*680000/PPM)+Math.round(volatileMobilityPpm*170000/PPM)-Math.round(x.interior.crustFractionPpm*80000/PPM),0,PPM);
...
function pressureProxyBand(pressure){
  if(!C.safe(pressure,0,12000000))return 'UNKNOWN';
  if(pressure<12000)return 'TRACE_OR_COLLISIONLESS_PROXY';
  if(pressure<150000)return 'THIN_COLUMN_PROXY';
  if(pressure<1000000)return 'MODERATE_COLUMN_PROXY';
  if(pressure<4000000)return 'DENSE_COLUMN_PROXY';
  return 'VERY_DENSE_COLUMN_PROXY';
}
function constituentState(x){
  const family=typeof x.atmosphere.compositionFamily==='string'?x.atmosphere.compositionFamily:'UNKNOWN',pressure=x.atmosphere.pressureProxyPpm;
  ...
  return C.freeze({supported:true,status:'PRESENT',family,pressureProxyPpm:pressure,meanMolecularMassMilliAmu:x.atmosphere.meanMolecularMassMilliAmu,constituents,constituentCount:constituents.length,shareClosurePpm:constituents.reduce((n,v)=>n+v.sharePpm,0),retrievalClaim:false,spectroscopicAbundanceClaim:false});
}
function historyContext(x,history){
  const rawBase=x.atmosphere.escape?.escapePpm,currentXuv=x.atmosphere.escape?.xuvMilliWm2;
...
function atmosphereState(planet,regime,options={}){
  const x=C.extract(planet);if(!x.supported)return x;
  const constituents=constituentState(x),escape=historyContext(x,options.xuvHistory),pressure=x.atmosphere.pressureProxyPpm,family=typeof x.atmosphere.compositionFamily==='string'?x.atmosphere.compositionFamily:'UNKNOWN';
  const collapse=x.atmosphere.collapsePotentialPpm,cloud=x.atmosphere.cloudCondensatePotentialPpm,outgassing=x.atmosphere.outgassingPpm,molecularMass=x.atmosphere.meanMolecularMassMilliAmu;
  if(!C.safe(collapse,0,PPM)||!C.safe(cloud,0,PPM)||!C.safe(outgassing,0,PPM)||!C.safe(molecularMass,constituents.supported?1:0))return C.freeze({version:VERSION,worldIdentity:x.worldIdentity,supported:false,status:'UNSUPPORTED',reason:'INVALID_ATMOSPHERIC_PROXY_STATE',regime:regime.regime,pressureProxyPpm:pressure,composition:constituents,retentionEscape:escape,authority:AUTHORITY,canonicalPromotion:false});
  ...
  return C.freeze({version:VERSION,worldIdentity:x.worldIdentity,supported:true,status:'PRESENT',regime:regime.regime,pressureProxyPpm:pressure,pressureProxyBand:pressureProxyBand(pressure),composition:constituents,
...
function radiativeState(planet,regime,atmosphere){
  const x=C.extract(planet);if(!x.supported)return x;
  const eq=x.formation.equilibriumTemperatureMilliK,greenhouse=x.atmosphere.greenhouseDeltaMilliK;
...
O.v2x05InteriorAtmosphere=Object.freeze({VERSION,AUTHORITY,FAMILY,pressureProxyBand,heatShares,interiorState,constituentState,historyContext,atmosphereState,radiativeState});
