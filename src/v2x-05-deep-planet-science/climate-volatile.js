...
function coldShare(phaseCoverage){const row=phaseCoverage.find(v=>v.phase==='COLD_CONDENSED_DOMINANT');return row?row.sharePpm:0;}
function climateEnvelope(planet,regime,radiative,atmosphere){
  const x=C.extract(planet);if(!x.supported)return x;
  const eq=x.formation.equilibriumTemperatureMilliK,surface=radiative?.modeledSurfaceTemperature?.temperatureMilliK;
  const base=Number.isSafeInteger(surface)?surface:eq;
  const obliquity=x.inputs.obliquityMilliDeg,ecc=x.inputs.eccentricityPpm,rotationMilliHours=x.inputs.rotationPeriodMilliHours;
  ...
  const pressure=x.atmosphere.pressureProxyPpm,cloud=x.atmosphere.cloudCondensatePotentialPpm;
  ...
  let min=3000000,max=20000,totalTemp=0,northTemp=0,southTemp=0,northCount=0,southCount=0;
  const seasonalSums=[0,0,0,0],seasonalCounts=[0,0,0,0],phaseNames=['LIQUID_WINDOW_CANDIDATE','ICE_FAVORED','COLD_CONDENSED_DOMINANT','VAPOR_THERMAL_STRESS','NO_RESOLVED_SURFACE'],phaseIndex=Object.freeze({LIQUID_WINDOW_CANDIDATE:0,ICE_FAVORED:1,COLD_CONDENSED_DOMINANT:2,VAPOR_THERMAL_STRESS:3,NO_RESOLVED_SURFACE:4}),phaseCounts=[0,0,0,0,0];
  for(const cell of cells){const t=cell.temperatureMilliK;min=Math.min(min,t);max=Math.max(max,t);totalTemp+=t;if(cell.latitudeDeg>0){northTemp+=t;northCount++;}else if(cell.latitudeDeg<0){southTemp+=t;southCount++;}const si=SEASONS.indexOf(cell.seasonPpm);seasonalSums[si]+=t;seasonalCounts[si]++;phaseCounts[phaseIndex[cell.phaseState]]++;}
  const mean=Math.round(totalTemp/cells.length),northMean=Math.round(northTemp/northCount),southMean=Math.round(southTemp/southCount),seasonalMeans=seasonalSums.map((sum,i)=>Math.round(sum/seasonalCounts[i])),seasonMin=Math.min(...seasonalMeans),seasonMax=Math.max(...seasonalMeans),phaseShares=C.normalizePpm(phaseCounts);
  const phaseCoverage=freeze(phaseNames.map((phase,i)=>freeze({phase,cellCount:phaseCounts[i],sharePpm:phaseShares[i]})));
  const aggregates=freeze({meanTemperatureMilliK:mean,northMeanTemperatureMilliK:northMean,southMeanTemperatureMilliK:southMean,hemisphericContrastMilliK:Math.abs(northMean-southMean),seasonalMeanTemperatureMilliK:freeze(seasonalMeans),seasonalMeanAmplitudeMilliK:seasonMax-seasonMin,liquidWindowSharePpm:phaseCoverage[0].sharePpm,iceFavoredSharePpm:phaseCoverage[1].sharePpm+coldShare(phaseCoverage),phaseCoverage});
  return freeze({version:VERSION,worldIdentity:x.worldIdentity,supported:true,status:'PRESENT',modelClass:'BOUNDED_LATITUDINAL_SEASONAL_TRANSPORT_PROXY',notGcm:true,weatherForecastClaim:false,empiricalClimateNormalClaim:false,
    ...
    deterministicArithmetic:'INTEGER_PPM_QUARTER_SEASON_KERNEL',latitudeBands:LATITUDES,seasonMarksPpm:SEASONS,cellCount:cells.length,maxCells:C.LIMITS.climateCells,cells:freeze(cells),temperatureRangeMilliK:freeze({min,max,amplitude:max-min}),aggregates,
...
function volatilePartition(planet,regime,radiative,climate){
  const x=C.extract(planet);if(!x.supported)return x;
  const a=x.atmosphere,vals=[a.initialInventoryUnits,a.escapedUnits,a.interiorUnits,a.surfaceCondensedUnits,a.atmosphereUnits];
  ...
  if(declaredRetained!==undefined&&(!C.safe(declaredRetained,0,C.LIMITS.exactInventoryUnits)||BigInt(declaredRetained)!==retainedBig))return C.unsupported(x.worldIdentity,'SOURCE_VOLATILE_RETAINED_LEDGER_NOT_CLOSED');
  ...
  if(regime.solidSurfaceModel){
    const rawModeledSurface=radiative?.modeledSurfaceTemperature?.temperatureMilliK,hasModeledSurface=Number.isSafeInteger(rawModeledSurface),modeledSurface=hasModeledSurface?rawModeledSurface:null,eq=x.formation.equilibriumTemperatureMilliK,t=hasModeledSurface?modeledSurface:eq;
  ...
  const shareNames=['ESCAPED','INTERIOR','ATMOSPHERE','SURFACE_LIQUID','CRYOSPHERE','DEEP_CONDENSED_OR_CLOUD'],shareValues=[escaped,interior,atmospheric,surfaceLiquid,cryosphereUnits,deepCondensedOrCloudReservoir],sharePpm=C.normalizePpm(shareValues),reservoirShares=freeze(shareNames.map((reservoir,i)=>freeze({reservoir,sharePpm:sharePpm[i]})));
  return freeze({version:VERSION,worldIdentity:x.worldIdentity,supported:true,status:'PRESENT',modelUnit:'NORMALIZED_VOLATILE_INVENTORY_UNIT',initialInventoryUnits:initial,
    ...
    closure:freeze({sumUnits:partitionSum,residualUnits:Number(initialBig-partitionSumBig),exactInternalClosure:partitionSumBig===initialBig,sourceLedgerClosureVerified:true,arithmeticSafeInteger:Number.isSafeInteger(partitionSum),reservoirShareClosurePpm:reservoirShares.reduce((n,v)=>n+v.sharePpm,0)}),reservoirShares,
...
function causalAttribution(planet,regime,interior,atmosphere,volatile){
  const x=C.extract(planet);if(!x.supported)return x;
  const irradiation=x.formation.irradiationPpm,gravity=x.gravity.surfaceGravityMilliMs2,pressure=x.atmosphere.pressureProxyPpm,heat=x.interior.heat?.heatIndexPpm,ice=x.composition.icePpm,light=x.composition.lightVolatilePpm,obliquity=x.inputs.obliquityMilliDeg;
  if(!C.safe(irradiation,0,4000000)||!C.safe(gravity,1,1000000)||!C.safe(pressure,0,12000000)||!C.safe(heat,0,PPM)||!C.safe(ice,0,PPM)||!C.safe(light,0,PPM)||!C.safe(obliquity,0,180000))return C.unsupported(x.worldIdentity,'INVALID_CAUSAL_ATTRIBUTION_INPUTS');
  const raw=[
    irradiation,
