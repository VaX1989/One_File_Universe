...
function routeGraph(production){
  ...
  const known=new Set(settlementIds),routes=a(production?.routes).filter(r=>known.has(t(r.from))&&known.has(t(r.to))&&i(r.capacityUnits)>0).slice(0,RESILIENCE_LIMITS.routes);
...
function networkResilience(production){
  ...
  const routes=graph.routes.map(r=>{const alternatePathAvailable=reachableWithoutEdge(graph,t(r.from),t(r.to),t(r.edgeId));const capacity=Math.max(0,i(r.capacityUnits)),used=Math.max(0,i(r.usedUnits)),spare=Math.max(0,capacity-used);const criticalityPpm=alternatePathAvailable?c(Math.floor((1000000-c(r.conditionPpm||0))*.35+c(r.utilizationPpm||0)*.25)):c(Math.floor(650000+c(r.utilizationPpm||0)*.2+c(r.degradationPpm||0)*.15));if(!alternatePathAvailable)critical.push(t(r.edgeId));return f({edgeId:t(r.edgeId),from:t(r.from),to:t(r.to),alternatePathAvailable,bridgeLike:!alternatePathAvailable,capacityUnits:capacity,usedUnits:used,spareCapacityUnits:spare,criticalityPpm,conditionPpm:c(r.conditionPpm||0),degradationPpm:c(r.degradationPpm||0),authority:K.AUTHORITY.DERIVED})});
  ...
  return f({contract:RESILIENCE_CONTRACT,status:'MODELED',routes,settlements,criticalRouteIds:critical.sort(),authority:K.AUTHORITY.DERIVED,bounded:true,limits:RESILIENCE_LIMITS,mutationPerformed:false,physicalTransportGeometryClaim:false});
}
...
function cloneScenarioState(state){
  return {...state,
    regions:a(state?.regions).map(x=>({...x})),settlements:a(state?.settlements).map(x=>({...x})),tradeEdges:a(state?.tradeEdges).map(x=>({...x})),technology:{...(state?.technology||{})},polities:a(state?.polities).map(x=>({...x,settlementIds:a(x.settlementIds).slice()})),infrastructure:a(state?.infrastructure).slice(0,RESILIENCE_LIMITS.scenarioInfrastructureAssets).map(x=>({...x})),history:state?.history?{...state.history,proposals:a(state.history.proposals).slice()}:state?.history
  };
}
...
function projectRecoveryEnvelope(state,economy,{edgeId=null,shockSeverityPpm=700000,maxEpochs=12}={}){
  maxEpochs=c(maxEpochs,1,RESILIENCE_LIMITS.projectionEpochs);shockSeverityPpm=c(shockSeverityPpm,0,950000);const edge=edgeId?a(state?.tradeEdges).find(x=>t(x.edgeId)===t(edgeId)):a(state?.tradeEdges)[0];
  if(!edge)return f({contract:RECOVERY_CONTRACT,status:'NO_MODELED_EDGE',trajectory:[],authority:K.AUTHORITY.MODEL_DERIVED_SIMULATION,counterfactual:true,mutationPerformed:false});
  const originalRows=infrastructureForEdge(state,edge);if(!originalRows.length)return f({contract:RECOVERY_CONTRACT,status:'NO_MODELED_INFRASTRUCTURE_FOR_EDGE',edgeId:t(edge.edgeId),trajectory:[],authority:K.AUTHORITY.MODEL_DERIVED_SIMULATION,counterfactual:true,mutationPerformed:false});
  const originalCondition=averageCondition(originalRows),scenario=cloneScenarioState(state),scenarioRows=infrastructureForEdge(scenario,edge),shockCondition=c(Math.floor(originalCondition*(1000000-shockSeverityPpm)/1000000),50000,originalCondition);for(const x of scenarioRows){x.conditionPpm=shockCondition;x.status=shockCondition<350000?'DAMAGED':t(x.status||'ACTIVE');x.lastActiveEpoch=i(state?.epoch||0)}
  const settlementIds=[t(edge.from),t(edge.to)].sort(),trajectory=[];let previousCondition=shockCondition,recoveredEpoch=null;
