...
function networkResilience(production){
  ...
  const settlements=[];for(const id of graph.settlementIds){const incident=[],criticalIncident=[];let capacity=0,spare=0;for(const r of routes){bump(counter);if(r.from!==id&&r.to!==id)continue;incident.push(r);capacity+=r.capacityUnits;spare+=r.spareCapacityUnits;if(r.bridgeLike)criticalIncident.push(r)}settlements.push(f({settlementId:id,degree:incident.length,criticalIncidentRouteIds:criticalIncident.map(r=>r.edgeId).sort(),aggregateRouteCapacityUnits:capacity,aggregateSpareCapacityUnits:spare,evidenceComplete:!a(production.routes).some(r=>(r.from===id||r.to===id)&&r.conditionPpm===null),isolationRiskPpm:a(production.routes).some(r=>(r.from===id||r.to===id)&&r.conditionPpm===null)?null:incident.length===0?1000000:c(Math.floor(criticalIncident.length*1000000/incident.length)),authority:K.AUTHORITY.DERIVED}))}
...
function averageCondition(rows){if(!rows.length||rows.some(x=>K.knownPpm(x.conditionPpm)===null))return null;return c(Math.floor(rows.reduce((n,x)=>n+x.conditionPpm,0)/rows.length))}
...
function projectRecoveryEnvelope(state,economy,{edgeId=null,shockSeverityPpm=700000,maxEpochs=12}={}){
  ...
  const originalRows=infrastructureForEdge(state,edge);if(!originalRows.length)return recoveryRefusal('NO_MODELED_INFRASTRUCTURE_FOR_EDGE',{edgeId:t(edge.edgeId)});const originalCondition=averageCondition(originalRows);if(originalCondition===null)return recoveryRefusal('INFRASTRUCTURE_CONDITION_UNKNOWN',{edgeId:t(edge.edgeId)});if(originalCondition<=0)return recoveryRefusal('NO_RECOVERABLE_BASELINE',{edgeId:t(edge.edgeId),originalConditionPpm:originalCondition});
