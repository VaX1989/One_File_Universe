...
function projectRecoveryEnvelope(state,economy,{edgeId=null,shockSeverityPpm=700000,maxEpochs=12}={}){
  ...
  const allInfrastructure=a(state?.infrastructure);if(allInfrastructure.length>RESILIENCE_LIMITS.scenarioInfrastructureAssets)return f({contract:RECOVERY_CONTRACT,status:'INFRASTRUCTURE_BOUND_EXCEEDED',edgeId:t(edge.edgeId),observedInfrastructureAssets:allInfrastructure.length,maxInfrastructureAssets:RESILIENCE_LIMITS.scenarioInfrastructureAssets,trajectory:[],authority:K.AUTHORITY.MODEL_DERIVED_SIMULATION,counterfactual:true,mutationPerformed:false});
  ...
  const originalCondition=averageCondition(originalRows);if(originalCondition<=0)return f({contract:RECOVERY_CONTRACT,status:'NO_RECOVERABLE_BASELINE',edgeId:t(edge.edgeId),originalConditionPpm:originalCondition,trajectory:[],authority:K.AUTHORITY.MODEL_DERIVED_SIMULATION,counterfactual:true,canonicalForecast:false,mutationPerformed:false});
  ...
  const settlementIds=[t(edge.from),t(edge.to)].sort(),trajectory=[];let recoveredEpoch=null;
