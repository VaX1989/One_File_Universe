...
if(!K||!I||!E||!S||!M)throw new Error('V2X-09 base module set required');
...
function advancedModule(){const A=O.v2x09CivilizationAdvanced;return A&&typeof A.modelAdvancedCivilization==='function'&&typeof A.networkResilience==='function'&&typeof A.projectRecoveryEnvelope==='function'?A:null}
function advancedRequired(contract,extra={}){return freeze({contract,status:'ADVANCED_MODEL_REQUIRED',authority:K.AUTHORITY.MODEL_DERIVED_SIMULATION,bounded:true,mutationPerformed:false,requiresAdvancedComposition:true,...extra,limitations:K.LIMITATIONS})}
...
function civilizationEmbodimentPacket(state,economy,{settlementId=null}={}){const A=advancedModule();if(!A)return M.civilizationEmbodimentPacket(state,{economy,advanced:null,settlementId});const advanced=A.modelAdvancedCivilization(state,economy);return M.civilizationEmbodimentPacket(state,{economy,advanced,settlementId});}
function advancedComposition(state,economy){const A=advancedModule();return A?A.modelAdvancedCivilization(state,economy):advancedRequired('ofu-v2x-09-advanced-civilization-composition-2',{status:'ADVANCED_MODEL_REQUIRED',production:null,society:null,urban:null,resilience:null,evidenceComplete:false,requiresConvergenceOwnerComposition:true});}
function networkResilience(production){const A=advancedModule();return A?A.networkResilience(production):advancedRequired('ofu-v2x-09-network-resilience-1',{routes:[],settlements:[],criticalRouteIds:[],operations:0,topologyOnly:true,physicalTransportGeometryClaim:false});}
function projectRecoveryEnvelope(state,economy,options={}){const A=advancedModule();return A?A.projectRecoveryEnvelope(state,economy,options):advancedRequired('ofu-v2x-09-recovery-envelope-1',{trajectory:[],reportedSubmodelOperations:0,counterfactual:true,canonicalForecast:false,canonicalHistoryMutation:false,planetOrLifeMutation:false});}
O.v2x09CivilizationEconomyCity=Object.freeze({VERSION:K.VERSION,ECONOMY_CONTRACT:K.ECONOMY_CONTRACT,MORPHOLOGY_CONTRACT:K.MORPHOLOGY_CONTRACT,IMPACT_CONTRACT:K.IMPACT_CONTRACT,FAILURE_CONTRACT,EMBODIMENT_CONTRACT:M.EMBODIMENT_CONTRACT,AUTHORITY:K.AUTHORITY,LIMITS:K.LIMITS,FAILURE_LIMITS,EMBODIMENT_LIMITS:M.EMBODIMENT_LIMITS,GOODS:K.GOODS,CAPABILITY_GRAPH:K.CAPABILITY_GRAPH,LIMITATIONS:K.LIMITATIONS,technologyProfile:K.technologyProfile,initializeEconomy:I.initializeEconomy,stepEconomy:E.stepEconomy,environmentImpactProposals:E.environmentImpactProposals,institutionProfile:S.institutionProfile,cityMorphology:M.cityMorphology,cityRenderPlan:M.cityRenderPlan,civilizationEmbodimentPacket,inspector:M.inspector,networkFailureEnvelope,advancedComposition,networkResilience,projectRecoveryEnvelope});
