...
function evidenceLayers(state,settlement,network,dynamics){
  ...
  for(const r of routeRowsFor(network,sid).filter(r=>r.evidenceClass==='MODELED_INFRASTRUCTURE_CONDITION'&&arr(r.sourceInfrastructureIds).length>0&&clamp(r.degradationPpm||0)>=450000)){const ids=[text(r.edgeId),...arr(r.sourceInfrastructureIds).map(text)];add('CORRIDOR_DEGRADATION','MODELED_INFRASTRUCTURE_CONDITION',ids,null,r.degradationPpm)}
  const pressure=arr(dynamics?.settlementPressures).find(x=>x.settlementId===sid);if(pressure&&!pressure.incompleteEvidence&&pressure.migrationPressurePpm>=550000)add('AGGREGATE_OUTMIGRATION_PRESSURE','DERIVED_AGGREGATE_PRESSURE',[sid],null,pressure.migrationPressurePpm);
  return layers;
}
function phaseFor(settlement,pressure,layers){
  ...
  if(pressure?.incompleteEvidence)return 'MODEL_EVIDENCE_INCOMPLETE';
...
function urbanEvolution(state,network,dynamics){
  ...
  return freeze({contract:CONTRACT,status:'MODELED',settlements:rows,renderCues,evidenceGaps:arr(dynamics.evidenceGaps),evidenceComplete:dynamics.evidenceComplete!==false,technology:tech,limits:LIMITS,authority:AUTHORITY.MODEL_DERIVED_SIMULATION,renderAuthority:AUTHORITY.PRESENTATION_ONLY,bounded:true,rendererMutationPerformed:false,sharedSceneMutationPerformed:false,requiresConvergenceOwnerComposition:true,physicalGeometryClaim:false,canonicalHistoryMutation:false,limitations:LIMITATIONS});
}
