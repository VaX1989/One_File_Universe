...
function polityInstitutionProposals(state,pressures,network){
  ...
  for(const p of sortId(arr(state?.polities),'polityId')){
    const members=arr(p.settlementIds).map(text).map(id=>byPressure.get(id)).filter(Boolean);if(!members.length)continue;const scarcity=clamp(Math.floor(members.reduce((n,x)=>n+x.scarcityPpm,0)/members.length)),routeStress=clamp(Math.floor(members.reduce((n,x)=>n+x.routeStressPpm,0)/members.length)),service=clamp(Math.floor(members.reduce((n,x)=>n+x.serviceSatisfactionPpm,0)/members.length)),legitimacy=clamp(p.legitimacyPpm||0),cohesion=clamp(p.cohesionPpm||0);const damaged=arr(network?.bottlenecks).filter(b=>b.kind==='INFRASTRUCTURE_DEGRADATION'&&members.some(m=>{const r=arr(network?.routes).find(x=>x.edgeId===b.edgeId);return r&&(r.from===m.settlementId||r.to===m.settlementId)}));
