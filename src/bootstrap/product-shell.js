...
function keyLabel(type,key){
 if(!type||!key)return type||'Canonical target';
 const val=n=>key[n]===undefined?'?':String(key[n]);
 if(type==='Planet')return `Planet · site ${val('siteX')},${val('siteY')},${val('siteZ')} · orbit ${val('orbitSlot')}`;
 if(type==='System')return `System · site ${val('siteX')},${val('siteY')},${val('siteZ')}`;
 if(type==='Star')return `Star · site ${val('siteX')},${val('siteY')},${val('siteZ')} · component ${val('componentIndex')}`;
 if(type==='Moon')return `Moon · orbit ${val('orbitSlot')} · satellite ${val('satelliteSlot')}`;
 return `${type} · ${Object.values(key).slice(-3).map(String).join(',')}`;
}
function reasonText(reason){
 const map={P3_ABSENT:'No canonical planet exists at this address.',P5_MASS_DOMAIN:'The canonical planet exists, but this P5 realization does not support its mass domain.',MASS_DOMAIN:'The canonical planet exists, but this P5 realization does not support its mass domain.',TEST_ONLY_NEGATIVE_FIXTURE:'Intentional negative rendering fixture.'};
 return map[reason]||('The canonical target exists, but the current rendering model cannot realize it ('+String(reason||'UNSUPPORTED')+').');
}
function sync(){
 ...
 const fullId=payload.planet||null,viewportLabel=P?.chosen?.key?keyLabel('Planet',P.chosen.key):'Canonical planet';
 ...
 if(payload.previewStatus==='SUPPORTED'){
  hidden('viewport-state',true);text('render-target-state','READY');q('render-target-state')?.setAttribute('data-tone','ok');
  if(P.environment){text('science-environment',P.environment.authority==='P5_CANONICAL'?'Canonical environment':'Environment');}
  if(P.eligibility){const label=P.eligibility.state==='INSUFFICIENT_ENVIRONMENT'?'Insufficient environment':P.eligibility.state;text('science-biology',label);}
 }else if(payload.previewStatus==='UNSUPPORTED'){
  hidden('viewport-state',false);text('viewport-state-title','Visualization unavailable');text('viewport-state-message',reasonText(payload.reason));text('render-target-state','UNSUPPORTED');q('render-target-state')?.setAttribute('data-tone','warn');text('science-biology','Not evaluated for this target');
 }
 if(payload.previewStatus!==state.lastTargetStatus||payload.reason!==state.lastTargetReason){state.lastTargetStatus=payload.previewStatus;state.lastTargetReason=payload.reason;if(payload.previewStatus==='UNSUPPORTED')announce('Visualization unsupported. '+reasonText(payload.reason));}
 if(payload.scale){const labels={SYSTEM_VIEW:'System',PLANET_ORBIT:'Orbit',PLANET_APPROACH:'Approach',SURFACE_LOCAL:'Surface'};text('scale-current',labels[payload.scale]||payload.scale);for(const b of document.querySelectorAll('[data-render-target]')){const name=b.dataset.scaleName||b.textContent.trim();b.setAttribute('aria-pressed',name===(labels[payload.scale]||'')?'true':'false')}}
 ...
 if(fullId&&fullId!==state.lastSelectedId){state.lastSelectedId=fullId;announce('Selected target '+short(fullId))}
}
...
function init(){
 ...
 document.addEventListener('click',e=>{const open=e.target.closest?.('[data-open-workspace]');if(open){workspace(open.dataset.openWorkspace,{focus:false});return}const tab=e.target.closest?.('[data-workspace]');if(tab){workspace(tab.dataset.workspace,{focus:false});return}},false);
