...
function sync(){
 ...
 if(payload.previewStatus==='SUPPORTED'){
  hidden('viewport-state',true);text('render-target-state','READY');q('render-target-state')?.setAttribute('data-tone','ok');text('viewport-status','The viewport shows a presentation-only realization of the currently selected canonical planet.');
  ...
  const reason=reasonText(payload.reason);hidden('viewport-state',false);text('viewport-state-title','Visualization unavailable');text('viewport-state-message',reason);text('viewport-status','Visualization unsupported for the selected canonical target. '+reason);text('render-target-state','UNSUPPORTED');q('render-target-state')?.setAttribute('data-tone','warn');text('science-environment','Not evaluated — target unsupported');text('science-biology','Not evaluated — target unsupported');
 }
