...
const state={version:VERSION,authority:AUTHORITY,ready:false,attachAttempts:0,focusAnnouncements:0,staleFocusClears:0,descriptionBindings:0,lastFocusedLabel:null,lastStage:null};
let runtime=null,canvas=null,status=null,observer=null;
...
function setStatus(text){if(!status||status.textContent===text)return;status.textContent=text}
...
function clearStaleFocus(snapshot){
 ...
 canvas.setAttribute('aria-label',BASE_LABEL);state.lastFocusedLabel=null;state.staleFocusClears++;setStatus(instruction(state.lastStage));
}
...
function snapshot(){return Object.freeze({...state,statusText:status?.textContent||null,canvasLabel:canvas?.getAttribute('aria-label')||null,describedBy:canvas?.getAttribute('aria-describedby')||null,keyShortcuts:canvas?.getAttribute('aria-keyshortcuts')||null})}
