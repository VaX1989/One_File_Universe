...
function bindKeyboard(){root.addEventListener('keydown',event=>{const canvas=document.getElementById('living-view');if(!runtime||document.activeElement!==canvas||event.altKey||event.ctrlKey||event.metaKey||event.shiftKey)return;if(event.key==='['){if(runtime.snapshot().historyDepth<=0)return;event.preventDefault();event.stopImmediatePropagation();state.keyboardBack++;runtime.back()}else if(event.key===']'){if(runtime.snapshot().forwardDepth<=0)return;event.preventDefault();event.stopImmediatePropagation();state.keyboardForward++;runtime.forward()}},false)}
function attach(){
