...
const state={version:VERSION,ready:false,attachStatus:'waiting',attachAttempts:0,decorations:0,forwardClicks:0,keyboardBack:0,keyboardForward:0,focusRestores:0,navigationFailures:0};
...
function navigationFailure(direction,error){state.navigationFailures++;console.error('Living '+direction+':',error);O.productUI?.announce?.(direction+' navigation could not be restored')}
function forwardButton(back){let b=panel?.querySelector('[data-living-action="forward"]');if(b)return b;b=document.createElement('button');b.type='button';b.className=back.className||'living-button';b.textContent='Forward';b.dataset.livingAction='forward';b.setAttribute('aria-keyshortcuts',']');b.addEventListener('click',()=>{if(!runtime?.snapshot?.().forwardDepth)return;state.forwardClicks++;restoreForwardFocus=true;try{runtime.forward()}catch(error){restoreForwardFocus=false;navigationFailure('Forward',error)}});back.after(b);state.decorations++;return b}
function decorate(){
...
function bindKeyboard(){root.addEventListener('keydown',event=>{const canvas=document.getElementById('living-view');if(!runtime||document.activeElement!==canvas||event.altKey||event.ctrlKey||event.metaKey||event.shiftKey)return;if(event.key==='['){if(runtime.snapshot().historyDepth<=0)return;event.preventDefault();event.stopImmediatePropagation();state.keyboardBack++;try{runtime.back()}catch(error){navigationFailure('Back',error)}}else if(event.key===']'){if(runtime.snapshot().forwardDepth<=0)return;event.preventDefault();event.stopImmediatePropagation();state.keyboardForward++;try{runtime.forward()}catch(error){navigationFailure('Forward',error)}}},false)}
function attach(){
