...
const state={ready:false,system:null,star:null,targets:[],selectedIndex:0,stage:'system',lastPreviewId:null,lastSystemId:null,lastRenderNotice:null};
...
function renderSelection(){
 ...
 const P=root.__OFU_PLANET_PREVIEW__,canShow=rendererCanShow(target),heading=q('explore-selected-heading'),context=q('explore-selected-context'),status=q('explore-selection-status'),go=q('explore-go');
 ...
 if(status){
  if(canShow)status.textContent='This world is connected to the current renderer and can be explored through scale.';
  else if(P?.targetStatus==='UNSUPPORTED'&&samePlanetKey(target.key,P?.chosen?.key))status.textContent='This canonical world cannot be visualized by the current renderer. Its scientific identity remains valid.';
  else status.textContent='This canonical world is selectable here. Persistent renderer retargeting awaits the shared navigation bridge.';
 }
...
function setStage(name,{announce=true}={}){
 ...
 if(announce)O.productUI?.announce?.(rendererCanShow(target)?stage.label+' view for '+target.name:stage.label+' selected; renderer unavailable for '+target.name);
 return true;
}
...
function sync(){
 ...
 if(previewId!==state.lastPreviewId){state.lastPreviewId=previewId;const currentIndex=state.targets.findIndex(target=>samePlanetKey(target.key,P?.chosen?.key));if(currentIndex>=0&&state.selectedIndex===currentIndex)renderSelection()}
 const notice=[P?.targetStatus,P?.targetReason,state.selectedIndex].join('|');if(notice!==state.lastRenderNotice){state.lastRenderNotice=notice;renderSelection()}
}
function init(){
...
O.v08ExploreNavigation=Object.freeze({seamVersion:1,state,STAGES,selectIndex,moveSelection,setStage,sync,rendererCanShow,planetName});
