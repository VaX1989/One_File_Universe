...
function drawChrome(s){
 ...
 for(const [scale,label] of Object.entries(labels)){const availability=runtime.navigationAvailability(scale),b=el('button',label,{type:'button','data-living-scale':scale,'aria-pressed':s.stage===scale});b.disabled=!availability.enabled;if(availability.reason)b.title=availability.reason;b.addEventListener('click',()=>act(()=>runtime.scale(scale)));rail.append(b);}
