...
function establishInspectorPlanet(key){
 ...
 type.value='Planet';type.dispatchEvent(new Event('change',{bubbles:true}));
 ...
 const selected=I.query();
 if(selected?.status!=='PRESENT'||I.state.current?.type!=='Planet'||!sameKey(I.state.current.key,k))throw new Error('Inspector canonical selection did not commit requested planet');
...
function selectPlanet(key,{announce=true}={}){
 ...
 const result=Object.freeze({
  ...
  canonicalDigest:selected.digest,
