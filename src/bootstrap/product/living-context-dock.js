...
const state={version:VERSION,authority:AUTHORITY,ready:false,attachStatus:'waiting',attachAttempts:0,decorations:0,legacyCorrections:0,runtimeUpdates:0,lastStage:null,lastIdentity:null,lastContextAuthority:null,inspectAvailable:false};
...
function decorate(){
 ...
 const copy=document.getElementById('v1x10-context-copy'),authority=dock.querySelector('.v1x10-context-authority'),inspect=[...dock.querySelectorAll('button')].find(button=>/inspect/i.test(button.textContent||''));if(!copy||!authority||!inspect)return false;
 ...
 const before=actualSignature(copy,authority,inspect),desired=desiredSignature(m);if(state.ready&&before!==desired)state.legacyCorrections++;
 ...
 if(changed)state.decorations++;state.lastStage=m.stage;state.lastIdentity=m.identity||null;state.lastContextAuthority=m.contextAuthority;state.inspectAvailable=m.inspectAvailable;state.ready=true;state.attachStatus='attached';return true;
}
function attach(){
