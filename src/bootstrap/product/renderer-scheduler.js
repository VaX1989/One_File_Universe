...
function installLivingNavigationPacer(){
 ...
 const originalCreate=living.create.bind(living),version='ofu-living-navigation-pacer-1',navigationStages=Array.from(living.NAVIGATION_STAGES||[]),runtimes=new Set(),pinchPointers=new Set();
 function create(...args){
  const runtime=originalCreate(...args);let frame=0,coordinate=null,options=null,queuedStage=null,events=0;
  const pacing={version,strategy:'RAF_LATEST_PINCH_COORDINATE_WITH_SYNC_BOUNDARIES_AND_TERMINAL_FLUSH',inputEvents:0,frames:0,boundaryCommits:0,terminalCommits:0,coalescedEvents:0,pendingEvents:0,staleDrops:0,cancelledEvents:0};
