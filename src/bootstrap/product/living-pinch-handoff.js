...
const VERSION='ofu-v11-living-pinch-handoff-1',MAX_ATTACH_ATTEMPTS=120;
const state={version:VERSION,ready:false,attachStatus:'waiting',attachAttempts:0,pinchStarts:0,handoffs:0,handoffMoves:0,coreOwnedMoves:0,cancellations:0,lastRemainingPointer:null,lastGesture:null};
...
function move(event){
 ...
 const dx=p.x-handoff.x,dy=p.y-handoff.y;handoff.x=p.x;handoff.y=p.y;
 if(!Number.isFinite(dx)||!Number.isFinite(dy)||(dx===0&&dy===0))return;
 renderer.rotate(dx,dy);state.handoffMoves++;state.lastGesture='handoff-drag';
}
function finish(event,cancelled=false){
 const hadPinch=pinchSeen||pointers.size>=2,known=pointers.delete(event.pointerId);if(!known)return;
 if(cancelled)state.cancellations++;
 if(pointers.size>=2){pinchSeen=true;handoff=null;return}
 if(pointers.size===1&&hadPinch){const [pointerId,p]=pointers.entries().next().value;handoff={pointerId,x:p.x,y:p.y};pinchSeen=false;state.handoffs++;state.lastRemainingPointer=pointerId;state.lastGesture=cancelled?'pinch-cancel-handoff':'pinch-handoff';return}
 pinchSeen=false;handoff=null;if(pointers.size===0)state.lastRemainingPointer=null;
}
function bind(){
 canvas.addEventListener('pointerdown',down,false);canvas.addEventListener('pointermove',move,false);canvas.addEventListener('pointerup',event=>finish(event,false),false);canvas.addEventListener('pointercancel',event=>finish(event,true),false);
...
const api=Object.freeze({VERSION,MAX_ATTACH_ATTEMPTS,state,snapshot:()=>Object.freeze({...state,activePointers:pointers.size,pinchSeen,handoffActive:!!handoff,handoffPointerId:handoff?.pointerId??null})});
