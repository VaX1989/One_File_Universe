...
const state={version:VERSION,ready:false,attachStatus:'waiting',attachAttempts:0,pinchStarts:0,handoffs:0,handoffMoves:0,coreOwnedMoves:0,cancellations:0,captureLosses:0,thresholdWaits:0,captureOwnedMoves:0,lastRemainingPointer:null,lastGesture:null};
...
// Handoff continuation is capture-owned. While the release dead-zone is active
// no later bubble listener (including stale/duplicate presentation adapters) may
// turn micro-jitter into a rotation. Once the threshold is crossed, this owner
// emits exactly one request through the canonical RAF-paced renderer and still
// stops the same pointermove from reaching competing drag owners.
function move(event){
...
function finish(event,cancelled=false){
 ...
 if(pointers.size===1&&hadPinch){const [pointerId,p]=pointers.entries().next().value;handoff={pointerId,startX:p.x,startY:p.y,x:p.x,y:p.y,engaged:false};pinchSeen=false;state.handoffs++;state.lastRemainingPointer=pointerId;state.lastGesture='pinch-handoff';return true}
...
const api=Object.freeze({VERSION,MAX_ATTACH_ATTEMPTS,DRAG_THRESHOLD_PX,state,snapshot:()=>Object.freeze({...state,activePointers:pointers.size,pinchSeen,handoffActive:!!handoff,handoffPointerId:handoff?.pointerId??null,handoffEngaged:!!handoff?.engaged,thresholdPx:DRAG_THRESHOLD_PX,captureOwned:true})});
