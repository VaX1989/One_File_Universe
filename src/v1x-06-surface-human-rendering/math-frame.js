...
function neighborTile(face,level,x,y,edge){const b=tileBounds(level,x,y),e=String(edge||'').toUpperCase();if(!['N','S','E','W'].includes(e))throw new RangeError('edge must be N/S/E/W');let u=b.uc,v=b.vc;if(e==='N')v=b.v1+b.span*.5;else if(e==='S')v=b.v0-b.span*.5;else if(e==='E')u=b.u1+b.span*.5;else u=b.u0-b.span*.5;const q=unitToFaceUv(faceUvRaw(face,u,v)),n=2**b.level,nx=Math.max(0,Math.min(n-1,Math.floor((q.u+1)*.5*n))),ny=Math.max(0,Math.min(n-1,Math.floor((q.v+1)*.5*n)));return freeze({face:q.face,level:b.level,x:nx,y:ny,key:tileKey(q.face,b.level,nx,ny),crossFace:q.face!==assertFace(face)})}
