...
async function autoMount(){
 ...
 let activationRequested=false;const bridge={setControls(value){const state=controller.snapshot();if(value?.enabled&&state.activated!==true&&!activationRequested){activationRequested=true;return controller.activate(value).finally(()=>{activationRequested=false;});}return controller.setControls(value);}};
 const controls=controlsApi.mount(section,bridge,{document:root.document,initial:{enabled:false,volume:0.5,muted:false,reducedSensory:false}});const tick=()=>{const s=controller.snapshot();if(s.activated&&s.controls.enabled&&!s.visibilitySuspended)controller.sync().catch(()=>{});};let timer=root.setInterval?root.setInterval(tick,600):null;const visibility=()=>controller.setVisibility(root.document.visibilityState!=='hidden').catch(()=>{});root.document.addEventListener('visibilitychange',visibility);visibility();
 mounted=Object.freeze({controller,controls,section,snapshot:()=>controller.snapshot(),activationPending:()=>activationRequested,dispose:async()=>{if(timer!==null&&root.clearInterval)root.clearInterval(timer);root.document.removeEventListener('visibilitychange',visibility);controls.dispose();if(section.parentNode)section.parentNode.removeChild(section);return controller.dispose();}});root.__OFU_V2X14_AUDIO__=mounted;return mounted;
}
O.v2x14LivingAudioController=Object.freeze({VERSION,AUTHORITY,create,autoMount,instance:()=>mounted});if(typeof root.document!=='undefined'){if(root.document.readyState==='loading')root.document.addEventListener('DOMContentLoaded',autoMount,{once:true});else if(root.setTimeout)root.setTimeout(autoMount,0);}
})(globalThis);
