...
const REGIME=Object.freeze({REGION:'galactic_region',NEIGHBORHOOD:'stellar_neighborhood'});
const regimeFor=stage=>REGIME[stage]||String(stage||'UNKNOWN').toLowerCase();
function fromLiving(s={}){
 const p=s.world?.planetology||{},local=s.local||null,stage=String(s.stage||'UNKNOWN'),atmosphere=p.atmosphere||{},micro=MICRO.has(stage),regime=regimeFor(stage);
 const pressure=Number(atmosphere.pressureProxyPpm),knownPressure=Number.isFinite(pressure),surfaceLike=!!local&&!VACUUM.has(stage)&&!micro,supportsSound=surfaceLike&&knownPressure&&pressure>=12000;
