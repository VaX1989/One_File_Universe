...
const MICRO=new Set(['MATERIAL','MICROSTRUCTURE','TISSUE','CELL','MOLECULAR','ATOMIC']),VACUUM=new Set(['UNIVERSE','GALAXY','REGION','NEIGHBORHOOD','SYSTEM','ORBIT']),SURFACE=new Set(['GLOBAL_SURFACE','REGIONAL_SURFACE','LOCAL_SURFACE','HUMAN']);
...
function fromLiving(s={}){
 const p=s.world?.planetology||{},local=s.local||null,stage=typeof s.stage==='string'&&s.stage.length?s.stage:'UNKNOWN',atmosphere=p.atmosphere||{},micro=MICRO.has(stage),surface=SURFACE.has(stage),regime=regimeFor(stage);
 ...
 const modeledCiv=s.world?.civilization?.state==='MODELED_CIVILIZATION'&&localObjects.some(x=>x?.kind==='SETTLEMENT'),modeledLife=s.world?.biology?.occupancy?.biosphereEstablished===true&&localLife.length>0,civPresent=surface&&modeledCiv,lifePresent=surface&&modeledLife;
