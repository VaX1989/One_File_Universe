...
function exportPortable(input){const p=payload(input),digest=O.canonical.digestObject(p),text=stableJson({format:'OFU-SAVE',schemaVersion:SCHEMA,payload:p,integrity:{algorithm:'SHA-256',digest}});if(enc.encode(text).length>MAX_SAVE_BYTES)fail('save exceeds limit');return text;}
