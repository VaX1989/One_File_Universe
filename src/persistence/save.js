...
function expectedIdentity(expected){if(!isRecord(expected))fail('expected identity must be an object');const allowed=new Set(['manifestHash','masterSeed256']),keys=dataKeys(expected,'expected identity');for(const k of keys)if(!allowed.has(k))fail('expected identity has unsupported fields');const out={};if(Object.hasOwn(expected,'manifestHash')){if(typeof expected.manifestHash!=='string'||!/^[0-9a-f]{64}$/i.test(expected.manifestHash))fail('invalid expected manifest');out.manifestHash=expected.manifestHash.toLowerCase();}if(Object.hasOwn(expected,'masterSeed256')){if(typeof expected.masterSeed256!=='string'||!/^[0-9a-f]{64}$/i.test(expected.masterSeed256))fail('invalid expected seed');out.masterSeed256=expected.masterSeed256.toLowerCase();}return out;}
...
function importPortable(text,expected={}){
  const required=expectedIdentity(expected);if(typeof text!=='string')fail('save must be text');assertSaveTextBound(text);assertSaveParseBound(text);let c;try{c=JSON.parse(text);}catch{fail('malformed save');}
  ...
  if(Object.hasOwn(required,'manifestHash')&&p.universeIdentity.generatorManifestHash!==required.manifestHash)fail('manifest mismatch');if(Object.hasOwn(required,'masterSeed256')&&p.universeIdentity.masterSeed256!==required.masterSeed256)fail('universe identity mismatch');return p;
}
