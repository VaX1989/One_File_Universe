...
const terrainProfile=(profile,radius)=>{const terrain=profile||{};return Object.freeze({planetaryWavelength:Number(terrain.planetaryWavelengthM)||radius*.24,macroWavelength:Number(terrain.macroWavelengthM)||76000,ridgeWavelength:Number(terrain.ridgeWavelengthM)||18000,highlandWavelength:Number(terrain.highlandWavelengthM)||2700,localWavelength:Number(terrain.localWavelengthM)||240,detailWavelength:Number(terrain.detailWavelengthM)||29,planetaryAmplitude:Number(terrain.planetaryAmplitudeM)||760,macroAmplitude:Number(terrain.macroAmplitudeM)||820,ridgeAmplitude:Number(terrain.ridgeAmplitudeM)||430,highlandAmplitude:Number(terrain.highlandAmplitudeM)||96,localAmplitude:Number(terrain.localAmplitudeM)||8.5,detailAmplitude:Number(terrain.detailAmplitudeM)||.72})};
...
export function createPlanetaryTerrainSampler({radiusM,seed,minimumWavelengthM=0,profile=null}={}){
  ...
  const terrain=terrainProfile(profile,radius),weights=Object.freeze({planetary:bandWeight(terrain.planetaryWavelength,cutoff),macro:bandWeight(terrain.macroWavelength,cutoff),ridge:bandWeight(terrain.ridgeWavelength,cutoff),highland:bandWeight(terrain.highlandWavelength,cutoff),local:bandWeight(terrain.localWavelength,cutoff),detail:bandWeight(terrain.detailWavelength,cutoff)}),base=hash(seed),layers=Object.freeze({planetary:base^0x9e3779b9,macro:base^0x243f6a88,ridge:base^0xb7e15162,highland:base^0xdeadbeef,local:base^0x85ebca6b,detail:base^0xc2b2ae35}),sample=(direction,wavelength,salt)=>valueNoise3(direction,radius/wavelength,salt);
  return bodyFixedUnit=>{const direction=normalize(bodyFixedUnit);let height=0;if(weights.planetary>0)height+=(sample(direction,terrain.planetaryWavelength,layers.planetary)*.72+sample(direction,terrain.planetaryWavelength*.43,layers.planetary^0x51ed270b)*.28)*terrain.planetaryAmplitude*weights.planetary;if(weights.macro>0)height+=sample(direction,terrain.macroWavelength,layers.macro)*terrain.macroAmplitude*weights.macro;if(weights.ridge>0)height+=(1-Math.abs(sample(direction,terrain.ridgeWavelength,layers.ridge))-.48)*terrain.ridgeAmplitude*weights.ridge;if(weights.highland>0)height+=sample(direction,terrain.highlandWavelength,layers.highland)*terrain.highlandAmplitude*weights.highland;if(weights.local>0)height+=sample(direction,terrain.localWavelength,layers.local)*terrain.localAmplitude*weights.local;if(weights.detail>0)height+=sample(direction,terrain.detailWavelength,layers.detail)*terrain.detailAmplitude*weights.detail;return height};
}
