...
export function createMaterializationCache({maxEntries=8,onDispose=value=>value?.dispose?.()}={}){
  ...
  const entries=new Map(),pinned=new Set(),evicted=[],disposalHistory=[];let epoch=0,hits=0,misses=0,evictions=0,disposals=0,disposalCallbacks=0,disposalErrors=0,disposedResources=0,disposedSessions=0,releasedRepresentations=0,rematerializations=0;
  const touch=entry=>{entry.lastUsed=++epoch;return entry.value};
  const disposeEntry=(key,reason)=>{const entry=entries.get(key);if(!entry)return false;entries.delete(key);let report=null,error=null;try{disposalCallbacks++;report=onDispose(entry.value,{key,kind:entry.kind,reason})||null;disposedResources+=Math.max(0,Number(report?.resources)||0);disposedSessions+=Math.max(0,Number(report?.sessions)||0);releasedRepresentations+=Math.max(0,Number(report?.representations)||0)}catch(cause){disposalErrors++;error=String(cause?.message||cause)}finally{disposals++}disposalHistory.push(Object.freeze({key,kind:entry.kind,reason,callbackInvoked:true,resources:Math.max(0,Number(report?.resources)||0),sessions:Math.max(0,Number(report?.sessions)||0),representations:Math.max(0,Number(report?.representations)||0),error}));if(disposalHistory.length>64)disposalHistory.shift();if(error)throw new Error(`Materialization disposal failed for ${key}: ${error}`);return true};
  const enforce=()=>{
  ...
  return Object.freeze({
    ...
    snapshot(){const rows=[...entries.values()].sort((a,b)=>a.lastUsed-b.lastUsed).map(entry=>Object.freeze({key:entry.key,kind:entry.kind,pinned:pinned.has(entry.key),lastUsed:entry.lastUsed}));return Object.freeze({contract:'ofu-spatial-materialization-cache-2',capacity:limit,size:entries.size,bounded:entries.size<=limit,pinned:Object.freeze([...pinned]),entries:Object.freeze(rows),metrics:Object.freeze({hits,misses,evictions,disposals,disposalCallbacks,disposalErrors,disposedResources,disposedSessions,releasedRepresentations,rematerializations}),evictedHistory:Object.freeze([...evicted]),disposalHistory:Object.freeze([...disposalHistory])})}
  });
}
