...
export function createOpenUniverseAuthority(root=globalThis,{ctx,seedKey,maxGalaxies=8,maxRegions=10,maxSystems=12,cacheEntries=12}={}){
  ...
  let path=Object.freeze({universe,galaxy:null,region:null,neighborhood:null,system:null,body:null,surface:null,sample:null}),focus=universe,revision=0,world=null,currentWorldCacheKey=null,currentGalaxyWindowCacheKey=null,activeGalaxyWindow=seedGalaxyWindow,galaxyDiscovery=null,galaxies=[],activeRegions=[],activeSystems=[],activeBodies=[],windowsVisitedTotal=0,lastFailure=null;
  ...
  function loadGalaxyWindow(window){
    const normalized=galaxyWindow(window),windowId=galaxyWindowKey(normalized),cacheKey='galaxy-window:'+windowId,context=cache.materialize(cacheKey,()=>{const discovery=AS.discoverGalaxies({ctx,universeNode:universe,window:normalized,cursor:0,limit:Math.max(3,maxGalaxies),maxProbes:4096}),includeSeed=sameGalaxyWindow(normalized,seedGalaxyWindow),nodes=[...(includeSeed?[seedGraph.galaxy]:[]),...discovery.galaxies.filter(node=>!includeSeed||idOf(node)!==idOf(seedGraph.galaxy))].slice(0,maxGalaxies);return Object.freeze({kind:'GALAXY_WINDOW',window:normalized,nodes:Object.freeze(nodes),discovery})},{kind:'GALAXY_WINDOW',pin:true});
