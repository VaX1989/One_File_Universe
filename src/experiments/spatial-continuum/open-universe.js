...
export function createOpenUniverseAuthority(root=globalThis,{ctx,seedKey,maxGalaxies=8,maxRegions=10,maxSystems=12,cacheEntries=12}={}){
  ...
  let path=Object.freeze({universe,galaxy:null,region:null,neighborhood:null,system:null,body:null,surface:null,sample:null}),focus=universe,revision=0,world=null,currentWorldCacheKey=null,lastFailure=null;
  ...
  const pinPath=()=>cache.setPinned(['universe:'+idOf(universe),...address().segments.slice(1).map(segment=>`${segment.kind.toLowerCase()}:${segment.id}`),currentWorldCacheKey].filter(key=>key&&cache.has(key)));
  ...
  function restore(checkpoint){
    if(checkpoint?.contract!=='ofu-open-universe-checkpoint-1')throw new TypeError('A valid open-universe checkpoint is required');path=Object.freeze({universe,galaxy:null,region:null,neighborhood:null,system:null,body:null,surface:null,sample:null});focus=universe;world=null;currentWorldCacheKey=null;lastFailure=null;
  ...
  function snapshot(){const currentAddress=address(),cacheState=cache.snapshot();return Object.freeze({contract:'ofu-open-universe-authority-2',revision,universeId:universe.universeId,focusId:idOf(focus),focusKind:String(focus.kind).toUpperCase(),stage:stageForPath(),path,currentAddress,visibleGalaxies:galaxies.length,selectableGalaxies:galaxies.length,catalogue:Object.freeze(catalogueFor()),worldIdentity:world?.bodyId||null,surfaceIdentity:world?.surfaceId||null,sampleIdentity:world?.sampleId||null,activeWorldCacheKey:currentWorldCacheKey,cache:cacheState,materialization:Object.freeze({state:lastFailure?'FAILED':world?'READY':'IDLE',blocker:lastFailure}),bounded:galaxies.length<=maxGalaxies&&cacheState.bounded,scientificAuthority:Object.freeze({macroIdentity:AUTHORITY.CANONICAL,macroPlacement:AUTHORITY.PRESENTATION_ONLY,surfaceCoordinates:AUTHORITY.MODEL_DERIVED,terrain:AUTHORITY.PRESENTATION_ONLY})})}
