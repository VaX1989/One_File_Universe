...
export function createLocalEnvironmentGenerator({seed,terrainSampler,regime={},cellSizeM=32,activeRadius=2,maxInstances=120}={}){
  ...
  const cell=(cellX,cellZ)=>{
    ...
    for(let index=0;index<count;index++){
      ...
      const tall=grammar==='ANGULAR_ICE_FRACTURE_FIELD',rounded=grammar==='ROUNDED_SEDIMENT_FIELD',ejecta=grammar==='ANGULAR_EJECTA_FIELD',base=tall?(.22+rnd()*.72):rounded?(.15+rnd()*.28):ejecta?(.12+rnd()*.5):(.16+rnd()*.56),vertical=tall?1.8+rnd()*3.8:grammar==='UPLIFTED_LITHIC_FIELD'?1.1+rnd()*2.3:rounded?(.45+rnd()*.36):(.65+rnd()*1.25),landmark=index===0&&((cellX%3+3)%3===0)&&((cellZ%3+3)%3===0)&&rnd()<Math.min(1,place.landmarkChance*2.4),hierarchyScale=landmark?1.8+rnd()*.75:1,depth=classifyHumanDepth({eastM:point.eastM,northM:point.northM,centerEastM:0,centerNorthM:0,cellSizeM:size}),depthScale=depth==='NEAR'?place.scaleBands.near:depth==='MID'?place.scaleBands.mid:place.scaleBands.far;
      instances.push(Object.freeze({id:`${id}:feature:${index}`,cellId:id,eastM:point.eastM,northM:point.northM,heightM:heightAt(point),scale:Object.freeze([base*(tall?.42:rounded?1.35:ejecta?1.45:1)*hierarchyScale*depthScale,base*vertical*hierarchyScale*depthScale,base*(tall?.26:rounded?1:ejecta?.68:1)*hierarchyScale*depthScale]),rotation:Object.freeze([(rnd()-.5)*(tall?.24:.9),rnd()*Math.PI*2,(rnd()-.5)*(tall?.2:.9)]),family:grammar,depthBand:depth,landmark,cluster:index%place.clusterAnchors.length,authority:'PRESENTATION_ONLY'}));
    }
