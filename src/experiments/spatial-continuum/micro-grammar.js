...
import { createEpistemicScalePresentation } from './epistemic-scale-presentation.js';
...
const stableElements=elements=>elements&&typeof elements==='object'?Object.fromEntries(Object.entries(elements).sort(([a],[b])=>a.localeCompare(b)).map(([key,value])=>[key,Number(value)||0])):null;

function contextualComponents(source){
  return Object.freeze((Array.isArray(source?.components)?source.components:[]).slice(0,16).map(component=>Object.freeze({id:String(component.id||'UNKNOWN'),ppm:Number(component.ppm)||0,formula:component.formula==null?null:String(component.formula),elements:component.elements&&typeof component.elements==='object'?Object.freeze(stableElements(component.elements)):null,authority:String(component.authority||source.chemistryAuthority||'UNRESOLVED_CHEMISTRY')})));
}

function sourceContextFingerprint({id,kind,phase,structure,components,porosityPpm,chemistryAuthority,source}){
  const token=JSON.stringify({id,kind,phase,structure,porosityPpm,chemistryAuthority,sourceEntityId:String(source?.sourceEntityId||''),provider:String(source?.provider||''),components:components.map(component=>({id:component.id,ppm:component.ppm,formula:component.formula,elements:component.elements,authority:component.authority}))});
  return hash(token).toString(16).padStart(8,'0');
}

export function createContextualMaterialGrammar({sampleId,sampleKind,source={},presentationSeed=null}={}){
  const id=String(sampleId||source.sourceEntityId||'unknown-sample'),kind=String(sampleKind||source.kind||'UNKNOWN').toUpperCase(),phase=String(source.phase||(kind==='WATER'?'LIQUID':'SOLID')).toUpperCase(),structure=String(source.structure||(kind==='ICE'?'CRYSTALLINE_PROXY':kind==='ROCK'?'POLYCRYSTALLINE_PROXY':kind==='SOIL'?'POROUS_AGGREGATE':phase==='LIQUID'?'FLUID':'UNRESOLVED')).toUpperCase(),family=FAMILY[kind]||'UNKNOWN_CONTEXT',topology=topologyFor(structure,phase),components=contextualComponents(source),porosityPpm=Math.max(0,Math.min(1000000,Number(source.porosityPpm)||0)),chemistryAuthority=String(source.chemistryAuthority||'UNRESOLVED_CHEMISTRY'),contextFingerprint=sourceContextFingerprint({id,kind,phase,structure,components,porosityPpm,chemistryAuthority,source}),seed=String(presentationSeed?`${presentationSeed}:${id}:${kind}:${contextFingerprint}`:`${id}:${kind}:${contextFingerprint}:contextual-material`),rnd=randomFactory(seed+':material'),domainCount=Math.max(1,Math.min(8,components.filter(component=>component.ppm>0).length||1)),instanceCount=topology==='LATTICE_ORIENTED_PRESENTATION'?27:topology==='POROUS_AGGREGATE_PRESENTATION'?36:topology==='DISPERSED_FIELD_PRESENTATION'?42:topology==='FLUID_NEIGHBORHOOD_PRESENTATION'?32:24+Math.floor(rnd()*13);
  return Object.freeze({contract:'ofu-r6-contextual-material-grammar-2',sourceSampleId:id,sourceKind:kind,sourceContextFingerprint:contextFingerprint,continuityKey:`sample:${id}`,family,phase,structure,topology,palette:Object.freeze(PALETTES[family]||PALETTES.UNKNOWN_CONTEXT),components,componentCount:components.length,domainCount,instanceCount,porosityPpm,chemistryAuthority,presentationSeed:seed,authority:AUTHORITY.PRESENTATION_ONLY,inputAuthority:String(source.authority||AUTHORITY.MODEL_DERIVED),claims:Object.freeze({exactSpecimenGeometry:false,exactGrainGeometry:false,exactPoreGeometry:false,exactPhaseArrangement:false}),deterministic:true});
}

function molecularMotifAnchors(grammar,rnd){
...
function atomicField(grammar,rnd){
  ...
  return Object.freeze({...specification,points:Object.freeze(points),pointCount:points.length,authority:AUTHORITY.PRESENTATION_ONLY,semantics:'SOURCE_TOPOLOGY_CONTEXT_NOT_PARTICLE_POSITIONS',countSemantics:'BOUNDED_RENDER_SAMPLES_NOT_PARTICLE_COUNT'});
}

export function createContextualMicroGrammar(options={}){
  const source=options.source||{},material=createContextualMaterialGrammar(options),rnd=randomFactory(material.presentationSeed+':micro'),motifAnchors=molecularMotifAnchors(material,rnd),microstructure=microstructurePoints(material,rnd),knownElements=Object.freeze([...new Set(material.components.flatMap(component=>component.elements?Object.keys(component.elements):[]))].sort()),field=atomicField(material,randomFactory(material.presentationSeed+':atomic-field')),epistemic=createEpistemicScalePresentation({sourceSampleId:material.sourceSampleId,sourceKind:material.sourceKind,sourceContextFingerprint:material.sourceContextFingerprint,source,topology:material.topology,chemistryAuthority:material.chemistryAuthority,knownElements,atomicField:field}),exactStructure=epistemic.molecular.exactStructure,positions=exactStructure?exactStructure.sites:motifAnchors,links=exactStructure?exactStructure.links:Object.freeze([]),exactRelations=Boolean(exactStructure&&links.length);
  return Object.freeze({contract:'ofu-r6-contextual-micro-grammar-2',sourceSampleId:material.sourceSampleId,sourceKind:material.sourceKind,sourceContextFingerprint:material.sourceContextFingerprint,continuityKey:material.continuityKey,family:material.family,phase:material.phase,structure:material.structure,topology:material.topology,palette:material.palette,material,epistemic,positions,links,siteSemantics:exactStructure?'AUTHORITATIVE_STRUCTURE_SITE':'ENSEMBLE_MOTIF_ANCHOR_NOT_ATOM',linkSemantics:'PRESENTATION_PROXIMITY_NOT_BOND',relationSemantics:exactStructure?'AUTHORITATIVE_SOURCE_RELATION':'NO_EXACT_RELATIONS_RENDERED',siteCount:positions.length,motifAnchors,microstructurePositions:microstructure,knownElements,elementContextAuthority:knownElements.length?material.chemistryAuthority:'UNKNOWN',atomicContextPoints:8+hash(material.presentationSeed+':atomic')%9,atomicField:field,corePointSemantics:'PRESENTATION_CONTEXT_NOT_NUCLEONS',presentationSeed:material.presentationSeed,authority:AUTHORITY.PRESENTATION_ONLY,scientistStatus:Object.freeze({molecular:epistemic.molecular.status,atomic:epistemic.atomic.status}),claims:Object.freeze({exactMolecularSpecies:false,exactMolecularArrangement:Boolean(exactStructure),exactMolecularBonds:exactRelations,exactNuclearComposition:false,exactElectronState:false,particlePositions:false,knownElementsAreArrangementClaim:false,quantumSimulation:false}),deterministic:true});
}
