...
export function createContextualMicroGrammar({sampleId,sampleKind,source={}}={}){
  const id=String(sampleId||'unknown-sample'),kind=String(sampleKind||source.kind||'UNKNOWN').toUpperCase(),family=FAMILY[kind]||'UNKNOWN_CONTEXT',rnd=randomFactory(`${id}:${kind}:contextual-micro`),siteCount=7+Math.floor(rnd()*6),positions=[];
  ...
  return Object.freeze({contract:'ofu-contextual-micro-grammar-1',sourceSampleId:id,sourceKind:kind,family,palette:Object.freeze(PALETTES[family]),positions:Object.freeze(positions),links:Object.freeze(links),siteCount,atomicContextPoints:8+hash(id+':atomic')%9,authority:AUTHORITY.PRESENTATION_ONLY,claims:Object.freeze({exactMolecularSpecies:false,exactMolecularArrangement:false,exactNuclearComposition:false,exactElectronState:false}),deterministic:true});
}
