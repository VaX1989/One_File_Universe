...
import { createContinuumRenderer } from './renderer-phase2.js';
...
export async function bootSpatialContinuum(){
  ...
  try{
    ...
    const orientation=createOrientationScaleUX({container:root.querySelector('#continuum-orientation'),reducedMotion:reduced,onIntent:intent=>{if(intent?.type==='BACK')goBack()}});
    ...
    function rebind(nextWorld=world,now=currentTime()){
      ...
      renderer.rebind(world);lastInputAt=now;reportAsync(resolveLocalDestinations());
    }
    async function resolveLocalDestinations(){
      if(!openUniverse.world)return null;const bodyId=openUniverse.world.bodyId,surfaceId=openUniverse.world.surfaceId,result=await openUniverse.discoverLocalDestinationsAsync({}, {onProgress:progress=>{if(openUniverse.world?.surfaceId===surfaceId)elements.movement.textContent=`Resolving nearby objects · ${progress.probes} of ${progress.maxProbes} model points`}});
      if(disposed||!['COMPLETED','CACHED'].includes(result.status)||openUniverse.world?.bodyId!==bodyId||openUniverse.world?.surfaceId!==surfaceId||!result.catalogue)return result;
      renderer.refreshLocalDestinations(result.catalogue);lastCatalogueSignature='';transitionLog.push({type:'LOCAL_DESTINATIONS_RESOLVED',bodyId,surfaceId,requestId:result.requestId,status:result.status,objects:result.catalogue.nodes.length,probes:result.probes,slices:result.slices,at:currentTime()});if(kernel.snapshot(currentTime()).scale.semanticStage==='HUMAN')announce(`${result.catalogue.nodes.length} nearby inspectable objects resolved`);return result;
    }
    ...
    function catalogueForUi(stage){
      if(stage==='HUMAN'&&openUniverse.world)return Object.freeze(openUniverse.localDestinationsSnapshot().nodes.map(object=>Object.freeze({...object,selected:sampleExplicitlySelected&&object.id===world.sampleId})));
    ...
    const api=Object.freeze({
      ...
      dispose(){if(disposed)return false;disposed=true;orientation.destroy();renderer.dispose();openUniverse.dispose();return true}
    });
