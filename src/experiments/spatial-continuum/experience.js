...
export async function bootSpatialContinuum(){
  ...
  try{
    ...
    const pushBranchCheckpoint=()=>{const now=currentTime(),entry=Object.freeze({open:openUniverse.checkpoint(),kernel:kernel.checkpoint(now),sampleExplicitlySelected});if(branchHistory.at(-1)?.open.address!==entry.open.address||branchHistory.at(-1)?.kernel.stage!==entry.kernel.stage)branchHistory.push(entry);if(branchHistory.length>64)branchHistory.shift();return entry};
