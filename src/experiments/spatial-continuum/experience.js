...
export async function bootSpatialContinuum(){
  ...
  try{
    ...
    let lastFrame=performance.now(),lastUi=0,lastRevision=-1,lastInputAt=null,lastStage='UNIVERSE',lastAnnouncement='',lastCatalogueSignature='',drag=null,pinch=null,disposed=false,visualReady=false,humanBranchCaptured=false,surfaceRebases=0;
    ...
    const pushBranchCheckpoint=({resetLocalPosition=false}={})=>{const now=currentTime(),captured=kernel.checkpoint(now),camera=resetLocalPosition?Object.freeze({...captured.camera,localPosition:Object.freeze([0,0,0])}):captured.camera,kernelCheckpoint=resetLocalPosition?Object.freeze({...captured,camera}):captured,entry=Object.freeze({open:openUniverse.checkpoint(),kernel:kernelCheckpoint,sampleExplicitlySelected});if(branchHistory.at(-1)?.open.address!==entry.open.address||branchHistory.at(-1)?.kernel.stage!==entry.kernel.stage)branchHistory.push(entry);if(branchHistory.length>64)branchHistory.shift();return entry};
    ...
    function frame(now){if(disposed)return;const interval=now-lastFrame;lastFrame=now;if(interval<1000){frameIntervals.push(interval);if(frameIntervals.length>2400)frameIntervals.shift()}let state=kernel.snapshot(now);if(state.scale.semanticStage==='HUMAN'&&keys.size){kernel.moveLocal((keys.has('w')?1:0)-(keys.has('s')?1:0),(keys.has('d')?1:0)-(keys.has('a')?1:0),Math.min(.05,interval/1000));state=kernel.snapshot(now);elements.movement.textContent='WASD movement · '+state.camera.localPosition.map(value=>value.toFixed(1)).join(', ')}state=rebaseHumanTravel(state,now);renderer.update(state);if(lastInputAt!=null&&state.revision!==lastRevision){responseTimes.push(Math.max(0,currentTime()-lastInputAt));lastInputAt=null}lastRevision=state.revision;if(now-lastUi>80){lastUi=now;updateUi(state)}requestAnimationFrame(frame)}
    const api=Object.freeze({
      contract:'ofu-spatial-continuum-open-universe-4',openUniverse,get world(){return world},kernel,renderer,reducedMotion:reduced,travelTo,travelBy,chooseDestination,chooseSurfaceTarget,chooseSample,back:goBack,moveLocal:(forward,right,dt=.1)=>{kernel.moveLocal(forward,right,dt);return kernel.snapshot(currentTime())},settle:()=>{const state=kernel.settle(currentTime());renderer.update(state);return state},
      resetMetrics(){frameIntervals.length=0;responseTimes.length=0;transitionLog.length=0;lastFrame=currentTime();return true},
      snapshot(){const state=kernel.snapshot(currentTime()),render=renderer.snapshot(),open=openUniverse.snapshot();return Object.freeze({contract:'ofu-spatial-continuum-open-snapshot-4',status:visualReady?'READY':'RENDERING',worldIdentity:open.worldIdentity,sourceSampleIdentity:sampleExplicitlySelected?world.sampleId:null,state,openUniverse:open,render,explorationFreedom:'BRANCHING_BUT_CONSTRAINED',surfaceRebases,branchHistory:Object.freeze({depth:branchHistory.length,capacity:64,bounded:branchHistory.length<=64,latestAddress:branchHistory.at(-1)?.open.address||null}),performance:Object.freeze({samples:frameIntervals.length,median:percentile(frameIntervals,.5),p95:percentile(frameIntervals,.95),p99:percentile(frameIntervals,.99),longFrames34:frameIntervals.filter(value=>value>34).length,longFrames50:frameIntervals.filter(value=>value>50).length,inputResponseMedian:percentile(responseTimes,.5)}),transitionLog:Object.freeze([...transitionLog]),interactionLog:Object.freeze([...interactionLog]),runtimeNetworkResources:performance.getEntriesByType('resource').filter(entry=>/^https?:/i.test(entry.name)).length,canonicalIdentityPreserved:open.currentAddress.includes(open.focusId),singleCameraAuthority:render.cameraCount===1})},
