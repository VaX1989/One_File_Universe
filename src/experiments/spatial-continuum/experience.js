...
export async function bootSpatialContinuum(){
  ...
  try{
    ...
    let lastFrame=performance.now(),lastUi=0,lastRevision=-1,lastInputAt=null,lastStage='UNIVERSE',lastAnnouncement='',lastCatalogueSignature='',drag=null,pinch=null,disposed=false,visualReady=false,initialFieldReady=false,humanBranchCaptured=false,surfaceRebases=0,evidenceClock=null;
    ...
    async function exploreGalaxyField(direction){
      ...
      try{
        ...
        const now=currentTime(),next=openUniverse.snapshot();sampleExplicitlySelected=false;kernel.rebind({graph:openUniverse.graph,frames:world.frames,targets:openUniverse.cameraTargets(world)},now);kernel.travelTo('UNIVERSE',now,{push:false});kernel.settle(now);lastCatalogueSignature='';lastInputAt=now;transitionLog.push({type:'GALAXY_WINDOW_STREAM',from:before,to:next.galaxyStreaming.activeWindowKey,requestId:result.requestId,status:result.status,direction:String(direction),durationMs:result.durationMs??0,at:now});announce(`Discovered galaxy field ${next.galaxyStreaming.activeWindowKey}`);return result;
    ...
    root.querySelector('[data-action="galaxy-previous"]').addEventListener('click',()=>reportAsync(exploreGalaxyField('negativeX')));
    root.querySelector('[data-action="galaxy-next"]').addEventListener('click',()=>reportAsync(exploreGalaxyField('positiveX')));
