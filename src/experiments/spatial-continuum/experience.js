...
export async function bootSpatialContinuum(){
  ...
  try{
    ...
    const finishPointer=event=>{const point=pointerPosition(event),wasDrag=drag&&drag.id===event.pointerId,moved=wasDrag&&drag.moved;pointers.delete(event.pointerId);if(canvas.hasPointerCapture?.(event.pointerId))try{canvas.releasePointerCapture(event.pointerId)}catch{}let hit=null;if(wasDrag&&!moved){hit=renderer.pick(point.x,point.y);if(hit){const stage=kernel.snapshot(currentTime()).scale.semanticStage;stage==='HUMAN'&&openUniverse.world?.local.objects.some(object=>object.entityId===hit.id)?chooseSample(hit.id):chooseDestination(hit.id)}}recordInteraction({type:'POINTER_FINISH',pointerId:event.pointerId,x:point.x,y:point.y,moved:!!moved,hitId:hit?.id||null,focusId:openUniverse.graph.focusId});if(pointers.size<2)pinch=null;if(wasDrag)drag=null};
    ...
    const api=Object.freeze({
      contract:'ofu-spatial-continuum-open-universe-4',openUniverse,get world(){return world},kernel,renderer,reducedMotion:reduced,travelTo,travelBy,chooseDestination,chooseSample,back:()=>kernel.back(currentTime()),settle:()=>kernel.settle(currentTime()),
