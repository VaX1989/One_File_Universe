...
function createP5Provider(ctx,physical,{presentationReliefFraction=DEFAULT_PRESENTATION_RELIEF_FRACTION,presentationElevationScale=null}={}){
 ...
 let scale;if(presentationElevationScale!==null&&presentationElevationScale!==undefined){if(!Number.isFinite(presentationElevationScale)||presentationElevationScale<0)throw new Error('invalid presentation elevation scale');scale=presentationElevationScale}else{if(!Number.isFinite(presentationReliefFraction)||presentationReliefFraction<0||presentationReliefFraction>MAX_PRESENTATION_RELIEF_FRACTION)throw new Error('invalid presentation relief fraction');scale=radius*presentationReliefFraction/Math.abs(ELEVATION_CODE_MIN)}
