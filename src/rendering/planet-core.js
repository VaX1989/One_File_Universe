...
function dynamicClip(radiusM,distanceM,surfaceRadiusM=radiusM){
 ...
 const altitude=Math.max(1,distanceM-surfaceRadiusM),near=Math.max(0.5,Math.min(10000,altitude*0.015)),horizon=Math.sqrt(Math.max(0,distanceM*distanceM-surfaceRadiusM*surfaceRadiusM)),far=Math.max(near*100,horizon+surfaceRadiusM*0.35);return Object.freeze({near,far,ratio:far/near,regime:'PLANET_LOCAL_FITTED',surfaceRadiusM});
}
