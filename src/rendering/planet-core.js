...
function normalize(v){const m=Math.hypot(v[0],v[1],v[2]);if(!m)throw new Error('zero direction');return Object.freeze([v[0]/m,v[1]/m,v[2]/m])}
...
function dynamicClip(radiusM,distanceM){const altitude=Math.max(1,distanceM-radiusM),near=Math.max(0.5,Math.min(10000,altitude*0.015)),horizon=Math.sqrt(Math.max(0,distanceM*distanceM-radiusM*radiusM)),far=Math.max(near*100,Math.min(radiusM*4,horizon+radiusM*0.25));return Object.freeze({near,far,ratio:far/near})}
function depthAdjudication(radiusM){const cases=[1.01,1.2,2,10,100].map(k=>{const c=dynamicClip(radiusM,radiusM*k);return Object.freeze({distanceRadii:k,near:c.near,far:c.far,ratio:c.ratio})});return Object.freeze({selected:'HIERARCHICAL_LOCAL_FRAME_DYNAMIC_CLIP',conventionalLocalFrame:Object.freeze({feasible:true,cases}),reversedZ:Object.freeze({feasible:false,reason:'WebGL2 lacks portable clip-control and default fixed-point depth does not justify added complexity'}),logarithmicDepth:Object.freeze({feasible:true,selected:false,reason:'shader complexity and interpolation cost unnecessary after local-frame dynamic clipping'}),hierarchicalMultipass:Object.freeze({feasible:true,selected:true,reason:'astronomical and terrain passes use bounded reference frames and fitted clip ranges'}),physicalGpuMemoryTelemetry:'UNAVAILABLE_PORTABLY'});}
