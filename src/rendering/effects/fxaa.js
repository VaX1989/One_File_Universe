(function(root){
'use strict';
const O=root.OFU=root.OFU||{},VERSION='ofu-render-fxaa-4',AUTHORITY='PRESENTATION_ONLY';
...
void main(){vec4 sampleM=texture(uColor,vUv);vec3 m=sampleM.rgb;vec3 n=texture(uColor,vUv+vec2(0.,-uInvResolution.y)).rgb;vec3 s=texture(uColor,vUv+vec2(0.,uInvResolution.y)).rgb;vec3 e=texture(uColor,vUv+vec2(uInvResolution.x,0.)).rgb;vec3 w=texture(uColor,vUv+vec2(-uInvResolution.x,0.)).rgb;float lm=luma(m),mn=min(lm,min(min(luma(n),luma(s)),min(luma(e),luma(w)))),mx=max(lm,max(max(luma(n),luma(s)),max(luma(e),luma(w))));float range=mx-mn;if(range<max(uEdgeThreshold,mx*0.0833)){outColor=vec4(m,sampleM.a);return;}vec2 dir=vec2(luma(w)-luma(e),luma(n)-luma(s));float d=max(abs(dir.x),abs(dir.y));dir=d>1e-5?dir/d:vec2(0.);vec3 a=.5*(texture(uColor,vUv+dir*uInvResolution*.333).rgb+texture(uColor,vUv-dir*uInvResolution*.333).rgb);outColor=vec4(mix(m,a,uSubpixel),sampleM.a);}`;
