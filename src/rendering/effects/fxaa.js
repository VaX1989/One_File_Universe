(function(root){
'use strict';
const O=root.OFU=root.OFU||{},VERSION='ofu-render-fxaa-1',AUTHORITY='PRESENTATION_ONLY';
function fail(c,m){const e=new Error(m);e.code=c;throw e}function clamp(x,a,b){x=Number(x);if(!Number.isFinite(x))fail('NUMBER','non-finite');return Math.max(a,Math.min(b,x))}
...
function referencePixel(sample,uv,invResolution,cfg=config()){if(typeof sample!=='function'||!Array.isArray(uv)||uv.length!==2||!Array.isArray(invResolution)||invResolution.length!==2)fail('INPUT','invalid reference input');const L=c=>c[0]*.299+c[1]*.587+c[2]*.114,S=(dx,dy)=>sample(uv[0]+dx*invResolution[0],uv[1]+dy*invResolution[1]).slice(0,3).map(x=>clamp(x,0,1));const m=S(0,0),n=S(0,-1),s=S(0,1),e=S(1,0),w=S(-1,0),lm=L(m),vals=[lm,L(n),L(s),L(e),L(w)],mn=Math.min(...vals),mx=Math.max(...vals),range=mx-mn;if(range<Math.max(cfg.edgeThreshold,mx*.0833))return Object.freeze(m);let dx=L(w)-L(e),dy=L(n)-L(s),d=Math.max(Math.abs(dx),Math.abs(dy));if(d<=1e-9)return Object.freeze(m);dx/=d;dy/=d;const a=S(dx*.333,dy*.333),b=S(-dx*.333,-dy*.333);return Object.freeze(m.map((x,i)=>x*(1-cfg.subpixel)+(a[i]+b[i])*.5*cfg.subpixel))}
