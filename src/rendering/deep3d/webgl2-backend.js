...
const VERSION='ofu-deep3d-webgl2-backend-1',AUTHORITY='PRESENTATION_ONLY',BACKEND='DEEP3D_WEBGL2_V2X13';
...
function compileDraw(packet){
 ...
 const positions=g.positions.map((v,i)=>Math.fround(finite(v,'positions['+i+']'))),lit=g.lit===true;
 let normals=null;if(lit){if(!Array.isArray(g.normals)||g.normals.length!==positions.length)throw new Error('lit Deep3D mesh normals required');normals=g.normals.map((v,i)=>Math.fround(finite(v,'normals['+i+']')))}
 let indices=null;if(g.indices!=null){if(!Array.isArray(g.indices))throw new Error('Deep3D indices must be an array');indices=g.indices.map((v,i)=>integer(v,'indices['+i+']',0,positions.length/3-1))}
 const m=packet.material||{};
 return Object.freeze({id:packet.packetId,domain,primitive,positions:Object.freeze(positions),normals:normals&&Object.freeze(normals),indices:indices&&Object.freeze(indices),lit,material:Object.freeze({baseColor:vec(m.baseColor||[0.5,0.5,0.5],3,'material.baseColor'),emissive:vec(m.emissive||[0,0,0],3,'material.emissive'),roughness:finite(m.roughness??0.65,'material.roughness'),metallic:finite(m.metallic??0,'material.metallic'),opacity:finite(m.opacity??1,'material.opacity'),pointSize:finite(m.pointSize??2,'material.pointSize')}),lod:Object.freeze({level:integer(g.lodLevel??0,'geometry.lodLevel',0,32),transition:finite(g.lodTransition??0,'geometry.lodTransition')})});
}
...
O.deep3dWebGL2Backend=Object.freeze({VERSION,AUTHORITY,BACKEND,DOMAINS,FAMILY_PRIMITIVE,compileDraw,compileScene,createBackend});
