...
function presentationBounds(draws){const min=[Infinity,Infinity,Infinity],max=[-Infinity,-Infinity,-Infinity];let samples=0;for(const draw of draws)for(let i=0;i<draw.positions.length;i+=3){for(let axis=0;axis<3;axis++){const value=draw.positions[i+axis];min[axis]=Math.min(min[axis],value);max[axis]=Math.max(max[axis],value)}samples++}if(!samples)return Object.freeze({authority:AUTHORITY,samples:0,min:Object.freeze([0,0,0]),max:Object.freeze([0,0,0]),span:Object.freeze([0,0,0]),nonDegenerateAxes:0,volumetric:false});const span=max.map((value,axis)=>value-min[axis]),largest=Math.max(...span,1),threshold=largest*1e-5,nonDegenerateAxes=span.filter(value=>value>threshold).length;return Object.freeze({authority:AUTHORITY,samples,min:Object.freeze(min),max:Object.freeze(max),span:Object.freeze(span),nonDegenerateAxes,volumetric:nonDegenerateAxes===3})}
