...
function create(options={}){
 ...
 function usage(name){const b=bucket(name);let bytes=0,objects=0,draws=0;for(const r of b.values()){bytes+=r.bytes;objects+=r.objects;draws+=r.draws}return{bytes,objects,draws}}
 function normalize(c){return{bytes:Math.max(0,Math.floor(Number(c?.bytes)||0)),objects:Math.max(0,Math.floor(Number(c?.objects)||0)),draws:Math.max(0,Math.floor(Number(c?.draws)||0))}}
 function fits(name,c){const u=usage(name),l=limits[name];return u.bytes+c.bytes<=l.bytes&&u.objects+c.objects<=l.objects&&u.draws+c.draws<=l.draws}
 function request(name,id,cost,priority=0){name=String(name).toUpperCase();id=String(id);const b=bucket(name),c=normalize(cost),l=limits[name];if(c.bytes>l.bytes||c.objects>l.objects||c.draws>l.draws){metrics.rejections++;return freeze({status:'REJECTED_OVERSIZE',name,id,cost:c,limit:l})}clock.tick++;b.delete(id);while(!fits(name,c)&&b.size){const victim=[...b.entries()].sort((a,b)=>a[1].priority-b[1].priority||a[1].lastUsed-b[1].lastUsed||a[0].localeCompare(b[0]))[0];b.delete(victim[0]);metrics.evictions++}if(!fits(name,c)){metrics.rejections++;return freeze({status:'REJECTED_CAPACITY',name,id})}b.set(id,{...c,priority:Number(priority)||0,lastUsed:clock.tick});metrics.allocations++;let total=0;for(const k of Object.keys(limits))total+=usage(k).bytes;metrics.peakBytes=Math.max(metrics.peakBytes,total);return freeze({status:'ALLOCATED',name,id,usage:usage(name)})}
 function release(name,id){const ok=bucket(name).delete(String(id));if(ok)metrics.releases++;return ok}
 function clear(reason='manual'){for(const b of buckets.values())b.clear();if(reason==='context-lost')metrics.contextLosses++;return snapshot()}
