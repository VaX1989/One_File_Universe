...
function upstreamIdentity(e,i){for(const k of ['canonicalId','entityId','id'])if(typeof e?.[k]==='string'&&e[k])return e[k];if(e?.canonicalKey)return JSON.stringify(e.canonicalKey,Object.keys(e.canonicalKey).sort());return'entity-'+i}
function normalizeEntities(entities){if(!Array.isArray(entities))throw new TypeError('entities array required');const seen=new Set(),out=[];for(let i=0;i<entities.length;i++){const e=entities[i],id=upstreamIdentity(e,i);if(seen.has(id))throw new Error('duplicate upstream identity '+id);seen.add(id);out.push(e)}return out}
function buildUniverse({scopeId,entities,cameraFrame,quality='STANDARD',presentationSeed='OFU-V2X03',morphology='UNKNOWN',densityHint=.5}={}){
 ...
 return freeze({version:VERSION,contract:CONTRACT,status:'READY',scale:'UNIVERSE',authority:AUTHORITY,objects:rep.objects,bounds:freeze({entities:rep.objects.length,maxEntities:cfg.entityLimit}),camera:freeze({consumedExternalFrame:true,ownsFrame:false}),claims:freeze({gridPrimary:false,physicalCoordinates:false,canonicalTruthChanged:false})});
}
function buildGalaxy({galaxy,entities=[],cameraFrame=null,quality='STANDARD',presentationSeed='OFU-V2X03',densityHint=.5}={}){
 if(!galaxy)throw new TypeError('galaxy required');const cfg=q(quality),galaxyId=upstreamIdentity(galaxy,0),morphology=galaxy?.metadata?.modelProfile?.morphology||galaxy?.facts?.morphology||galaxy?.morphology||'UNKNOWN',field=F.build({galaxyId,morphology,quality:cfg.field,presentationSeed});let rep=null;
