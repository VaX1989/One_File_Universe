...
const VERSION='ofu-v2x-03-macrocosm-batches-2',AUTHORITY='PRESENTATION_ONLY';
...
function packFloat7(items,role){
 ...
 for(const item of items){const p=item.position||{};out[k++]=Number(p.x)||0;out[k++]=Number(p.y)||0;out[k++]=Number(p.z)||0;out[k++]=Number(item.size??item.radius??1)||1;out[k++]=Number(item.brightness??item.opacity??item.weight??1)||1;out[k++]=Number(item.density??0)||0;out[k++]=Number(item.componentCode??0)||0}
