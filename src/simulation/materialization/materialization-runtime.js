(function(root){
'use strict';
const O=root.OFU=root.OFU||{},C=O.pxContracts,R=O.v2x01Contracts,K=O.v2x01CacheKey,S=O.v2x01AdaptiveScheduler,L=O.v2x01ResourceLedger,W=O.v2x01WorkerExecutor,V='ofu-v2x01-materialization-runtime-1';
...
function create(o={}){
  const b=R.budgets(o.budgets||{}),scheduler=o.scheduler||S.create({budgets:b}),ledger=o.ledger||L.create({budgets:b}),workers=o.workerExecutor===false?null:(o.workerExecutor||(W?W.create({maxPayloadBytes:b.taskBytes}):null)),providers=new Map(),entries=new Map(),active=new Map(),gen=new Map(),max=Math.min(b.cacheEntries,b.materializations);
  let seq=0,clock=0;
  const m={requests:0,hits:0,misses:0,materializations:0,evictions:0,invalidations:0,cancellations:0,staleRejects:0,contextLosses:0,memoryPressureEvents:0,restorations:0,workerResults:0,fallbackResults:0};
  function reg(x){
    C.keys(x,['providerId','domain','modelVersion','representationVersion','load'],['release','workerProgram']);
    const id=C.token(x.providerId),v=Object.freeze({...x,providerId:id,modelVersion:C.version(x.modelVersion),representationVersion:C.version(x.representationVersion),handlerId:'v2x01.materializer.'+id});
    C.assert(typeof x.load==='function'&&!providers.has(id),'SCHEMA','materializer');providers.set(id,v);workers?.register({id:v.handlerId,version:v.modelVersion,direct:(p,s)=>v.load(p,s),workerProgram:x.workerProgram||null});return v;
  }
  function norm(x){
    C.keys(x,['durable','targetState','taskClass','semantic','presentation','estimate'],['preferWorker']);
  ...
  function release(e,why){try{e.provider.release?.(e.value,{identity:e.identity,cacheDigest:e.cacheDigest},why)}catch{}ledger.release(e.resourceId);entries.delete(e.cacheDigest);m.evictions++}
  ...
  async function request(x){
    ...
    try{
      h=scheduler.schedule({id:jid,taskClass:r.taskClass,state:r.targetState,preemptible:r.taskClass!=='INTERACTION',run:async signal=>{
        ...
        let mode='DIRECT',raw;
        if(workers){const ex=await workers.execute({handlerId:r.provider.handlerId,payload,signal,preferWorker:r.preferWorker&&!!r.provider.workerProgram});mode=ex.mode;raw=ex.value;mode==='WORKER'?m.workerResults++:m.fallbackResults++}else raw=await r.provider.load(payload,signal);
        if(signal.aborted||gen.get(k)!==g){try{r.provider.release?.(raw,{identity:r.durable.identity},'stale-work')}catch{}m.staleRejects++;throw fail('STALE_WORK','superseded work')}
        C.keys(raw,['identity','semanticDigest','state','cpuEstimateBytes','gpuEstimateBytes','entities','operations','transferBytes','value']);C.assert(C.stable(R.identity(raw.identity))===C.stable(r.durable.identity)&&raw.semanticDigest===r.semanticDigest,'SEMANTIC_INVARIANCE','identity/digest');
        ...
        ledger.commit(rid,actual);room(k);const e={cacheDigest:r.cache.digest,logicalKey:k,provider:r.provider,identity:r.durable.identity,state:r.targetState,pinned:r.targetState==='IMMEDIATE',lastUse:++clock,resourceId:rid,usage:actual,value:C.data(raw.value,{bytes:b.taskBytes,nodes:4096}),executionMode:mode};entries.set(e.cacheDigest,e);m.materializations++;return witness(r,e,false,mode);
      }});
    ...
    try{return await h.promise}finally{if(active.get(k)?.jobId===jid)active.delete(k);if(!entries.has(r.cache.digest))ledger.release(rid)}
  }
  async function reconcileWorkingSet(xs){C.assert(Array.isArray(xs)&&xs.length<=max,'BUDGET','working set');const want=new Set(xs.map(x=>logical(norm(x))));for(const e of [...entries.values()])if(!want.has(e.logicalKey)){e.pinned=false;release(e,'working-set')}return Promise.allSettled(xs.map(request))}
  ...
  function snapshot(){const states=Object.fromEntries(R.STATES.map(s=>[s,0]));for(const e of entries.values())states[e.state]++;return C.data({version:V,workingSet:{entries:entries.size,active:active.size,states},metrics:m,scheduler:scheduler.snapshot(),resources:ledger.snapshot(),workers:workers?workers.snapshot():null},{bytes:262144,nodes:8192})}
  async function drain(){await scheduler.drain();return snapshot()}
  return Object.freeze({VERSION:V,registerMaterializer:reg,request,reconcileWorkingSet,restoreWorkingSet,handleMemoryPressure,handleContextLoss,pauseBackground,invalidate:x=>invalidateLogical(R.logicalIdentityKey(x),'explicit'),snapshot,drain});
}
