...
const VERSION='ofu-v2x01-adaptive-scheduler-2';
...
function create(options={}){
  ...
  const queues=new Map(R.TASK_CLASSES.map(k=>[k,[]])),jobs=new Map(),activeJobs=new Map(),waiters=[],classDispatches=new Map(R.TASK_CLASSES.map(k=>[k,0]));
  ...
  const metrics={submitted:0,started:0,completed:0,failed:0,cancelled:0,backpressure:0,preemptionSignals:0,agedPromotions:0,peakActive:0,peakQueued:0,peakJobs:0,startedByClass:Object.fromEntries(R.TASK_CLASSES.map(k=>[k,0])),startedByState:Object.fromEntries(R.STATES.map(k=>[k,0]))};
  const queuedCount=()=>[...queues.values()].reduce((n,q)=>n+q.length,0);
  function snapshot(){return C.data({version:VERSION,budget:{active:maxActive,queue:maxQueue},fairness:{classWeights:R.CLASS_WEIGHTS,stateStarvationBound:R.STATE_STARVATION_BOUND},current:{active:activeJobs.size,queued:queuedCount(),jobs:jobs.size},metrics});}
