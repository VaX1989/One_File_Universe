...
const VERSION='ofu-v2x01-adaptive-scheduler-3';
...
function create(options={}){
  ...
  const queues=new Map(R.TASK_CLASSES.map(k=>[k,[]])),jobs=new Map(),activeJobs=new Map(),admissions=new Set(),waiters=[],classDispatches=new Map(R.TASK_CLASSES.map(k=>[k,0]));
  ...
  const metrics={submitted:0,started:0,completed:0,failed:0,cancelled:0,backpressure:0,preemptionSignals:0,agedPromotions:0,admissionsReserved:0,admissionsConsumed:0,admissionsReleased:0,peakAdmissions:0,peakActive:0,peakQueued:0,peakJobs:0,startedByClass:Object.fromEntries(R.TASK_CLASSES.map(k=>[k,0])),startedByState:Object.fromEntries(R.STATES.map(k=>[k,0]))};
  const queuedCount=()=>[...queues.values()].reduce((n,q)=>n+q.length,0);
  function snapshot(){return C.data({version:VERSION,budget:{active:maxActive,queue:maxQueue},fairness:{classWeights:R.CLASS_WEIGHTS,stateStarvationBound:R.STATE_STARVATION_BOUND},current:{active:activeJobs.size,queued:queuedCount(),admissions:admissions.size,jobs:jobs.size},metrics});}
  ...
  function selectFromClass(klass){
    const q=queues.get(klass);if(!q.length)return null;const dispatched=classDispatches.get(klass);
    let bestIndex=-1,best=null;
    for(let i=0;i<q.length;i++){
      const job=q[i],aged=dispatched-job.classDispatchAtEnqueue>=R.STATE_STARVATION_BOUND;
    ...
    if(best.aged)metrics.agedPromotions++;return q.splice(bestIndex,1)[0];
  }
  function selectNext(){
  ...
  function start(job){
    ...
    classDispatches.set(job.taskClass,classDispatches.get(job.taskClass)+1);job.status='ACTIVE';activeJobs.set(job.id,job);metrics.started++;metrics.startedByClass[job.taskClass]++;metrics.startedByState[job.state]++;peaks();
  ...
  function schedule(input){
    C.assert(input&&typeof input==='object'&&!Array.isArray(input),'SCHEMA','scheduler task');C.keys(input,['id','taskClass','state','run'],['preemptible','admission']);
    const id=C.token(input.id,'task id'),taskClass=R.taskClass(input.taskClass),state=R.state(input.state);C.assert(typeof input.run==='function','SCHEMA','task run');if(input.preemptible!==undefined)C.assert(typeof input.preemptible==='boolean','SCHEMA','preemptible');C.assert(!jobs.has(id),'DUPLICATE','task '+id);
    ...
    const job={id,taskClass,state,run:input.run,preemptible:input.preemptible!==false,controller,resolve,reject,promise,status:'QUEUED',sequence:sequence++,classDispatchAtEnqueue:classDispatches.get(taskClass)};
