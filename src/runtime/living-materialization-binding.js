...
function create(canvas,glCanvas,options={}){
 ...
 async function render(s){
  if(disposed)throw new Error('Living V2X01 runtime binding disposed');
  const revision=Math.max(0,Number(s?.revision)||0);if(revision<latestRevision)return;
  latestRevision=revision;const sequence=++renderSequence;
  try{await prepare(s)}catch(error){if(sequence!==renderSequence||revision<latestRevision)return;failures++;throw error}
  if(sequence!==renderSequence||revision<latestRevision)return;
