(function(root){
'use strict';
const O=root.OFU=root.OFU||{},C=O.pxContracts,R=O.v2x01Contracts,V='ofu-v2x01-worker-executor-5';
...
function create(o={}){
  C.keys(o,[],['maxPayloadBytes','maxChunkBytes','maxStreamBytes','maxStreamChunks','timeoutMs']);
  const maxPayload=R.integer(o.maxPayloadBytes??R.DEFAULTS.taskBytes,'worker payload',1,R.HARD.taskBytes),maxChunk=R.integer(o.maxChunkBytes??maxPayload,'worker stream chunk',1,R.HARD.taskBytes),maxStreamBytes=R.integer(o.maxStreamBytes??R.DEFAULTS.transferBytes,'worker stream bytes',1,R.HARD.transferBytes),maxChunks=R.integer(o.maxStreamChunks??256,'worker stream chunks',1,4096),timeout=R.integer(o.timeoutMs??10000,'worker timeout',1,120000),h=new Map(),m={workerRuns:0,fallbackRuns:0,cancelled:0,timeouts:0,failures:0,streamWorkerRuns:0,streamFallbackRuns:0,streamChunks:0,streamAcks:0,streamBytes:0,peakStreamBytes:0,streamFailures:0};
  function register(x){
  ...
  function accountChunk(chunk,runBytes){const bytes=ENCODER.encode(JSON.stringify(chunk)).length,total=runBytes+bytes;C.assert(total<=maxStreamBytes,'STREAM_BUDGET','stream bytes');m.streamBytes+=bytes;m.peakStreamBytes=Math.max(m.peakStreamBytes,total);return total}
  ...
  async function directStream(v,p,onChunk,s){
    ...
    const started=Date.now();let it=null,index=0,runBytes=0;
    ...
    try{
      ...
      for(;;){const step=await guarded(it.next(),'stream next');if(step.done){const value=C.data(step.value,{bytes:maxPayload,nodes:4096});return Object.freeze({mode:'STREAM_FALLBACK',value,chunks:index,bytes:runBytes})}C.assert(index<maxChunks,'STREAM_BUDGET','stream chunks');const chunk=C.data(step.value,{bytes:maxChunk,nodes:4096});runBytes=accountChunk(chunk,runBytes);m.streamChunks++;await guarded(onChunk(chunk,index),'stream backpressure');m.streamAcks++;index++}
    }catch(e){m.streamFailures++;try{await it?.return?.()}catch{}throw e}
  }
  function workerStream(v,p,onChunk,s){
    m.streamWorkerRuns++;
    const source="'use strict';const H=("+v.workerStreamProgram+"),MAX_CHUNK="+maxChunk+",MAX_STREAM="+maxStreamBytes+",MAX_CHUNKS="+maxChunks+",ENC=new TextEncoder();let ACK=null;self.onmessage=async e=>{if(e.data&&e.data.type==='ACK'){if(ACK&&ACK.index===e.data.index){const r=ACK.resolve;ACK=null;r()}return}if(!e.data||e.data.type!=='START')return;try{const value=await H(e.data.payload),it=(value&&value[Symbol.asyncIterator]?value[Symbol.asyncIterator]():value&&value[Symbol.iterator]?value[Symbol.iterator]():null);if(!it||typeof it.next!=='function')throw Error('stream handler must return iterable');let i=0,total=0;for(;;){const step=await it.next();if(step.done){self.postMessage({type:'DONE',value:step.value,chunks:i,bytes:total});break}if(i>=MAX_CHUNKS)throw Error('stream chunks');const text=JSON.stringify(step.value);if(typeof text!=='string')throw Error('stream chunk JSON');const bytes=ENC.encode(text).length;if(bytes>MAX_CHUNK||total+bytes>MAX_STREAM)throw Error('stream bytes');total+=bytes;self.postMessage({type:'CHUNK',index:i,value:step.value,bytes});await new Promise(resolve=>{ACK={index:i,resolve}});i++}}catch(x){self.postMessage({type:'ERROR',error:String(x&&x.message||x)})}}",url=root.URL.createObjectURL(new root.Blob([source],{type:'text/javascript'}));
    return new Promise((res,rej)=>{let w,done=false,expected=0,runBytes=0,processing=Promise.resolve();const finish=(ok,x)=>{if(done)return;done=true;clearTimeout(t);s?.removeEventListener?.('abort',ab);try{w?.terminate()}catch{}try{root.URL.revokeObjectURL(url)}catch{}ok?res(x):rej(x)},ab=()=>{m.cancelled++;finish(false,err('CANCELLED','worker stream'))},t=setTimeout(()=>{m.timeouts++;finish(false,err('STREAM_TIMEOUT',v.id))},timeout);s?.addEventListener?.('abort',ab,{once:true});if(s?.aborted)return ab();try{
      w=new root.Worker(url);w.onerror=e=>{m.streamFailures++;finish(false,err('WORKER_STREAM_FAILURE',e?.message||v.id))};w.onmessage=e=>{const d=e.data||{};if(d.type==='CHUNK'){processing=processing.then(async()=>{if(done)return;C.assert(d.index===expected,'STREAM_ORDER','worker stream chunk order');C.assert(expected<maxChunks,'STREAM_BUDGET','stream chunks');const chunk=C.data(d.value,{bytes:maxChunk,nodes:4096});const before=runBytes;runBytes=accountChunk(chunk,runBytes);C.assert(d.bytes===runBytes-before,'STREAM_ORDER','worker stream byte count');m.streamChunks++;await onChunk(chunk,expected);if(done)return;m.streamAcks++;w.postMessage({type:'ACK',index:expected});expected++}).catch(error=>{m.streamFailures++;finish(false,error)});return}if(d.type==='DONE'){processing.then(()=>{if(done)return;C.assert(d.chunks===expected,'STREAM_ORDER','worker stream final count');C.assert(d.bytes===runBytes,'STREAM_ORDER','worker stream final bytes');const value=C.data(d.value,{bytes:maxPayload,nodes:4096});finish(true,Object.freeze({mode:'WORKER_STREAM',value,chunks:expected,bytes:runBytes}))}).catch(error=>{m.streamFailures++;finish(false,error)});return}if(d.type==='ERROR'){m.streamFailures++;finish(false,err('WORKER_STREAM_FAILURE',d.error||v.id))}};w.postMessage({type:'START',payload:p})
  ...
  function snapshot(){return C.data({version:V,limits:{maxPayloadBytes:maxPayload,maxChunkBytes:maxChunk,maxStreamBytes,maxStreamChunks:maxChunks,timeoutMs:timeout},available:{worker:!!root.Worker,blobWorker:!!(root.Worker&&root.Blob&&root.URL?.createObjectURL&&root.URL?.revokeObjectURL)},handlers:[...h.values()].map(x=>({id:x.id,version:x.version,workerCapable:!!x.workerProgram,streamCapable:!!x.workerStreamProgram})),metrics:m})}
