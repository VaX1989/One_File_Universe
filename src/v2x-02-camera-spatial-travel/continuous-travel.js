...
function strictJson(value,label='context',{maxBytes=4096,maxDepth=12,maxNodes=256,maxKeys=64}={}){
 ...
 function walk(v,depth,path){
  ...
  if(Array.isArray(v)){
   ...
   for(const key of keys){const descriptor=Object.getOwnPropertyDescriptor(v,key);if(!descriptor||!('value'in descriptor)||descriptor.get||descriptor.set)throw new TypeError(label+' contains accessor-backed value at '+path+'.'+key);out[key]=walk(descriptor.value,depth+1,path+'.'+key);}
  }
...
function createController({camera,reducedMotion=false,maxPendingLogDelta=.9,maxLookRadians=.35,maxTranslationUnits=1e6,maxContextDepth=64,maxContextBytes=8192,maxReverseBytes=65536,maxReverseOperations=128,maxJsonDepth=12,maxJsonNodes=256,linePixels=DEFAULT_LINE_PIXELS,pagePixels=DEFAULT_PAGE_PIXELS,maxWheelPixels=DEFAULT_MAX_WHEEL_PIXELS}={}){
 ...
 let trackedCommandCount=camera.snapshot().commandCount;
 ...
 function digestMatches(c,d,eps=1e-9){return c.pose.frameId===d.frameId&&c.semanticScale===d.semanticScale&&Math.abs(c.logDistanceM-d.logDistanceM)<=eps&&arraysClose(c.pose.position,d.position,eps)&&arraysClose(c.pose.orientation,d.orientation,eps);}
 ...
 function synchronizeExternal(source='controller-sync'){const c=camera.snapshot();if(c.commandCount!==trackedCommandCount){invalidateReverse('external-camera-command:'+String(source),{external:true});trackedCommandCount=c.commandCount;}return c;}
 function noteCamera(){trackedCommandCount=camera.snapshot().commandCount;}
 function record(entry){const value=Object.freeze(entry),bytes=utf8Bytes(JSON.stringify(value));if(journal.length>=journalLimit||anchorBytes+journalBytes+bytes>reverseByteLimit)invalidateReverse('reverse-journal-capacity');if(bytes>reverseByteLimit){lastReverseInvalidation=Object.freeze({reason:'single-reverse-operation-exceeds-budget',external:false,cameraCommandCount:camera.snapshot().commandCount});return;}journal.push(Object.freeze({value,bytes}));journalBytes+=bytes;}
 ...
 function captureReverseAnchor(label='travel-anchor'){
  ...
  while(reverseAnchors.length>=contextLimit||anchorBytes+journalBytes+bytes>reverseByteLimit){const evicted=reverseAnchors.shift();anchorBytes-=evicted.bytes;metrics.reverseEvictions++;}
 ...
 function popReverseAnchor(){const entry=reverseAnchors.pop()||null;if(entry)anchorBytes-=entry.bytes;sequence++;return entry?.value||null;}
