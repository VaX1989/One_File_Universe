...
function render(input,{camera,viewport={width:1280,height:720},orbitSegments,approach}={}){
  ...
  const projectedOrbits=scene.orbits.map(o=>freeze({canonicalEntityId:o.canonicalEntityId,geometryAuthority:o.geometryAuthority,currentOrbitalPhaseAuthority:o.currentOrbitalPhaseAuthority,points:o.vertices.map(p=>M.ndcToPixel(M.project(M.transform(p,scene.approach.systemOriginInParentFrame3d,scene.approach.presentationScale),camera),viewport)).filter(Boolean)}));
  const hitTargets=[];for(const node of scene.nodes){const pixel=M.ndcToPixel(M.project(node.transitionPosition3d,camera),viewport);if(!pixel)continue;hitTargets.push(freeze({canonicalEntityId:node.canonicalEntityId,kind:node.kind,parentCanonicalEntityId:node.parentCanonicalEntityId,x:pixel.x,y:pixel.y,depth:pixel.depth,visible:pixel.visible,radiusPx:node.kind==='STAR'?11:7,selectionAuthority:'UPSTREAM_SELECTION_CONTRACT'}))}
