...
function responsiveCamera(camera,viewport){
  ...
  const factor=Math.min(2.4,targetAspect/aspect),position=M.v3(camera.position,'camera.position'),target=M.v3(camera.target,'camera.target'),offset=M.sub(position,target);
  return freeze({...camera,position:M.add(target,M.mul(offset,factor)),responsiveViewportFit:{authority:'PRESENTATION_ONLY',sourceAspect:aspect,targetAspect,distanceFactor:factor}});
}
function render(input,{camera,viewport={width:1280,height:720},orbitSegments,approach}={}){
  ...
  const projectionCamera=responsiveCamera(camera,viewport);
