(function(root){
'use strict';
const O=root.OFU=root.OFU||{}; const S=()=>O.sha256; const enc=new TextEncoder();
...
function i64(n){n=BigInt(n);assert(n>=-0x8000000000000000n&&n<=0x7fffffffffffffffn,'i64 range');return u64(BigInt.asUintN(64,n));}
...
function address(segments){assert(Array.isArray(segments)&&segments.length>0&&segments.length<=64,'invalid address segments');const out=[Uint8Array.of(1),u16(segments.length)];for(const seg of segments){assert(seg&&typeof seg==='object','invalid address segment');if(seg.kind==='u64')out.push(Uint8Array.of(1),u64(seg.value));else if(seg.kind==='text')out.push(Uint8Array.of(2),text(seg.value));else if(seg.kind==='i64')out.push(Uint8Array.of(3),i64(seg.value));else throw new Error('unsupported address kind');}return concat(...out);}
...
O.canonical={u16,u32,u64,i64,concat,text,seed,address,derive32,deriveU32,canonicalObject,digestObject};
