...
| Matter / `MATTER` | material components | optical-stage framing; layered structural hierarchy | dark neutral, mineral spectral accents | source identity + regime honesty | deeper to molecular / back exact source | bounded components | instant regime change |
| Tissue / Cell | not a distinct Living stage in current authenticated source | **not fabricated**; represented only if provider exposes it through existing micro contracts later | — | — | — | — | — |
| Microscopic | current micro pipeline where represented | structural focus, not space-particle reuse | dark optical-stage | source + structure | deeper/back | bounded objects | no drift |
