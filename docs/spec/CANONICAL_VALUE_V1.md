...
OFU-CBV-1 is a small purpose-built deterministic TLV encoding selected over deterministic CBOR for P2 because its admitted domain and parser can be completely specified and independently implemented in a few hundred lines without an external dependency. This choice is versioned and does not claim general CBOR interoperability.

Tags: `00` null, `01` false, `02` true, `10` negative signed integer, `11` non-negative unsigned integer, `20` byte string, `21` UTF-8 text, `30` array, `31` text-key map. Lengths and integer magnitudes use minimal unsigned LEB128. Negative integers encode `(-n*2)-1`. Floating point is not admitted.
...
Malformed lengths, truncated values, non-minimal varints, unknown tags, trailing bytes, unsupported in-memory values, cycles and invalid Unicode fail closed. The encoding is intended to be injective over its admitted canonical value domain.
