...
| Phase-owned conformance evidence | Integration Board + phase CI owner | all phases | `phase-evidence-architecture-v1` | STABLE_SNAPSHOT | `main` executes explicit frozen P2 scope, packages only P2-owned evidence and rejects injected foreign phases; downstream phases use their own evidence roots. | main `20f152448013fdb4a2a840e428701e17294ecea0`; P4 `f189c82f15defddcc4f9468043c3d4df5f85462e` |
| Baseline vs mutable authority | Integration Board boundary; owning domains authoritative | P3/P4/P5+ | `baseline-mutable-authority-v1-snapshot` | STABLE_SNAPSHOT | Procedural baseline + P4 history + versioned domain transitions = current persistent world. | P3 `768ceb2…`, P4 `f189c82…`, P5 `efe2b8c…` |
...
| P4 Temporal Contract | P4 | P5, P6, P7+ | `ofu-p4-temporal-v1` candidate | PROPOSED | Evidence isolation and executable P2→P4 identity/byte-authority tests are resolved. Live frontier, checkpoint+bounded-tail mutation and reducer semantic authority remain material blockers. | `f189c82f15defddcc4f9468043c3d4df5f85462e` |
| P4 -> downstream transition ownership | P4 time/replay + domain transition owner | P5+ | snapshot | STABLE_SNAPSHOT | P4 owns time/order/replay/checkpoint/lineage; domain phases own transition physics/semantics. | P4 `f189c82…` |
...
| --- | --- | --- | --- |
| P2 / integration main | `main` | `20f152448013fdb4a2a840e428701e17294ecea0` | FROZEN / CLOSED; phase-independent evidence hardening merged and exact-main Foundation/P1/P2 jobs/aggregates PASS |
...
| P4 candidate | `feature/p4-temporal-kernel` | `f189c82f15defddcc4f9468043c3d4df5f85462e` | CONTINUE WITH MATERIAL FIXES; executable P2→P4 contract tests merged, merge-head certification running/required |
...
Currently executable and independently verified on a pre-merge exact P4 candidate:

- P2→P4 wrong-universe fail-closed (`INT-P2-P4-001`);
- P2 canonical-byte authority for P4 archives (`INT-P2-P4-002`);
- phase-evidence isolation including synthetic foreign-phase rejection.
