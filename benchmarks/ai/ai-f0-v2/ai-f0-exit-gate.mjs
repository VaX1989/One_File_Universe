...
export function adjudicateAiF0Exit(x){
  const keys=['assetSet','ortBundlePin','packageEvidence','runtimeAdmission','fallbackDeterministic','browserMatrix','modelResearchGate','soakResearchGate','standardEditionUnaffected'];
  if(!exactKeys(x,keys)||typeof x.fallbackDeterministic!=='boolean'||typeof x.standardEditionUnaffected!=='boolean'||!x.assetSet||!x.ortBundlePin||!x.packageEvidence||!x.runtimeAdmission||!x.browserMatrix||!x.modelResearchGate||!x.soakResearchGate)throw new Error('AI_F0_EXIT_SCHEMA');
  const fail=reason=>Object.freeze({schema:'ofu-ai-f0-exit-adjudication-4',met:false,reason,shippingPromotion:false,nextPhaseActivationAuthorized:false});
  ...
  if(x.packageEvidence.ok!==true||x.packageEvidence.reason!=='DETERMINISTIC_ZERO_FETCH_PACKAGE_STRUCTURALLY_VERIFIED')return fail('ZERO_FETCH_PACKAGE_EVIDENCE_REQUIRED');
  if(x.packageEvidence.shippingPromotion!==false||x.packageEvidence.directFileRuntimeEvidence!==false)return fail('PACKAGE_EVIDENCE_AUTHORITY_VIOLATION');
  ...
  return Object.freeze({schema:'ofu-ai-f0-exit-adjudication-4',met:true,reason:'AI_F0_EXIT_GATE_MET_RESEARCH_ONLY',shippingPromotion:false,nextPhaseActivationAuthorized:false});
}
