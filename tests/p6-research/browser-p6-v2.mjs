...
await browser.close();result.hostPlatform=process.platform;result.hostArch=process.arch;fs.mkdirSync('dist/evidence/p6',{recursive:true});const evidenceName=`browser-${process.platform}-${process.arch}-${name}.json`;fs.writeFileSync(`dist/evidence/p6/${evidenceName}`,JSON.stringify(result,null,2));console.log(JSON.stringify(result,null,2));
