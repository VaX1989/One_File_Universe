...
const evidencePath=path.join(root,'reports/v1x-03-macrocosm-rendering/resource-evidence.json');
const authoring=process.argv.includes('--write')||process.argv.includes('--regenerate');
const assert=(ok,message)=>{if(!ok)throw new Error('resource evidence verification: '+message)};
...
function deterministicProjection(value){return {status:value.status,contract:value.contract,authority:value.authority,canonicalPromotion:value.canonicalPromotion,scope:value.scope,measurements:value.measurements.map(m=>({inputCount:m.inputCount,quality:m.quality,iterations:m.iterations,frameHash:m.frameHash,counters:m.counters}))}}
if(authoring){fs.mkdirSync(path.dirname(evidencePath),{recursive:true});fs.writeFileSync(evidencePath,JSON.stringify(result,null,2)+'\n')}else{
 assert(fs.existsSync(evidencePath),'tracked evidence file missing');
 const before=fs.readFileSync(evidencePath);
 const tracked=JSON.parse(before);
 assert(JSON.stringify(deterministicProjection(tracked))===JSON.stringify(deterministicProjection(result)),'deterministic frame/resource projection drift; run explicit --write only after adjudicating the change');
 for(const m of result.measurements){assert([m.minMs,m.medianMs,m.p95Ms,m.maxMs,m.heapUsedDeltaBytes,m.rssDeltaBytes].every(Number.isFinite),'runtime measurement must be finite');assert(m.minMs>=0&&m.medianMs>=0&&m.p95Ms>=0&&m.maxMs>=0,'runtime duration must be non-negative')}
 const after=fs.readFileSync(evidencePath);assert(before.equals(after),'verification mode mutated tracked evidence');
}
console.log(JSON.stringify({...result,verificationMode:authoring?'AUTHORING_REGENERATION':'EXACT_READ_ONLY',trackedEvidenceMutated:authoring,deterministicProjectionVerified:!authoring}));
