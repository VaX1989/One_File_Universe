...
function run(cmd,args){
  const r=spawnSync(cmd,args,{
    encoding:'utf8',
    env:{...process.env,OFU_SOURCE_SHA:process.env.OFU_SOURCE_SHA||'LOCAL-UNPINNED'}
  });
  if(r.status!==0){
  ...
  assert.equal(evidence.corpus_digest,manifest.corpus_digest);
  return evidence;
}
...
assert.ok(js.positive_checks>=40);
assert.ok(js.rejection_checks>=35);
assert.ok(py.positive_checks>=40);
assert.ok(py.rejection_checks>=30);
assert.equal(js.p4_live_frontier_scenarios,p4.live_frontier_scenarios.length);
assert.equal(py.p4_live_frontier_scenarios,p4.live_frontier_scenarios.length);
assert.equal(js.p4_live_frontier_rejections,py.p4_live_frontier_rejections);
assert.equal(py.p4_frontier_model,'INDEPENDENT_MINIMAL_ORDER_KEY_STATE_MACHINE');

console.log(JSON.stringify({
  schema:'ofu-ind-conf-a-combined-test-v2',
  status:'PASS',
  corpus_digest:manifest.corpus_digest,
  live_frontier_scenarios:p4.live_frontier_scenarios.length,
  js,
  python:py
}));
