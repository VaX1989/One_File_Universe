...
assert.equal(d.version,'2.0.0');assert.equal(V.VERSION,'ofu-v2x02-continuous-travel-2');assert.equal(V.PROVIDER_ID,id);assert.equal(V.AUTHORITY,'PRESENTATION_ONLY');
...
for(const result of [pixel,line]){assert.equal(result.provider,id);assert.equal(result.version,'2.0.0');assert.deepEqual(result.selection,captured.selection);assert.equal(result.value.distanceAuthorityKey,'logDistanceM');assert.equal(result.value.semanticDistanceContract,S.VERSION);assert(result.value.toLogDistanceM>result.value.fromLogDistanceM);assert.equal(result.value.canonicalMutation,false);assert.equal(result.value.selectionMutation,false);assert.equal(result.value.historyMutation,false);assert.equal(result.value.cameraPathIsPhysicalTrajectory,false);}
...
console.log(JSON.stringify({status:'PASS',oracle:'v2x02-registry-binding-oracle-2',provider:id,version:d.version,admitted:true,bound:true,semanticDistanceDelegated:true,rawWheelParity:true,inputBounded:true,selectionInvariant:true,canonicalMutation:false,historyMutation:false,authority:'PRESENTATION_ONLY'}));
