...
assert(snapshot.order.includes(id),'provider descriptor must be admitted into the sealed graph when the V2X-02 runtime is explicitly selected');
assert(snapshot.bound.includes(id),'selected continuous-travel provider must bind before registry seal');
assert(plan.some(c=>c.id==='v2x02.camera.continuous-travel'));
assert(plan.some(c=>c.id==='v2x.frontier.providers.v2x02-camera-spatial-travel'),'V2X-02 explicit runtime selection must resolve through its frontier catalog, not the active PX shipping namespace');
assert(!plan.some(c=>c.id==='px.providers.v2x02-camera-spatial-travel'),'legacy active shipping catalog id must remain absent');
const componentManifest=JSON.parse(fs.readFileSync('config/components/v2x-02-camera-spatial-travel.json','utf8'));
const frontierCatalog=componentManifest.components.find(c=>c.id==='v2x.frontier.providers.v2x02-camera-spatial-travel');
assert(frontierCatalog,'V2X-02 manifest must expose the frontier provider catalog');
assert.equal(frontierCatalog.owner,'v2x-02-camera-spatial-travel');
const livingBinding=componentManifest.components.find(c=>c.id==='v2x02.camera.living-binding');
assert(livingBinding,'V2X-02 manifest must expose the convergence-owned Living binding');
