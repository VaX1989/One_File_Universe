...
for(const name of names){
  ...
  try{
    ...
    const ev=await page.evaluate(async()=>{
      ...
      const streamExecutor=OFU.v2x01WorkerExecutor.create({maxStreamChunks:8,maxChunkBytes:1024,maxStreamBytes:99,timeoutMs:5000});
    ...
    assert.equal(ev.streamResult.mode,'WORKER_STREAM');assert.equal(ev.streamResult.chunks,3);assert.equal(ev.streamResult.bytes,99);assert.deepEqual(ev.streamResult.value,{done:true,id:'browser-stream'});assert.deepEqual(ev.streamed,[{index:0,value:0},{index:1,value:1},{index:2,value:2}]);assert.equal(ev.streamSnapshot.limits.maxStreamBytes,99);assert.equal(ev.streamSnapshot.metrics.streamWorkerRuns,1);assert.equal(ev.streamSnapshot.metrics.streamChunks,3);assert.equal(ev.streamSnapshot.metrics.streamAcks,3);assert.equal(ev.streamSnapshot.metrics.streamBytes,99);assert.equal(ev.streamSnapshot.metrics.peakStreamBytes,99);assert.equal(ev.streamSnapshot.metrics.streamFailures,0);
