...
const cp=T.checkpoint({universeIdentity:u1,lineage:l1,baseline:{seed:'A'},events:[]});
...
assert.throws(()=>T.exportArchive({universeIdentity:u1,lineage:l1,baseline:{seed:'B'},checkpoint:cp,events:[]}),/archive checkpoint baseline mismatch/);
const archive=T.exportArchive({universeIdentity:u1,lineage:l1,baseline:{seed:'A'},checkpoint:cp,events:[]});assert.doesNotThrow(()=>T.importArchive(archive));
