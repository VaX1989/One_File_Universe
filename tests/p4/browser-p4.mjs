...
if(process.platform!==expectedPlatform)throw new Error(`platform mismatch: expected ${expectedPlatform}, got ${process.platform}`);
if(process.arch!==expectedArch)throw new Error(`architecture mismatch: expected ${expectedArch}, got ${process.arch}`);
if(!({chromium,firefox,webkit})[browserName])throw new Error('unsupported browser '+browserName);
...
function scenario(){
  ...
  const r=T.replay({universeIdentity:universe,lineage,events}),sorted=T.sortEvents(events),cp=T.checkpoint({universeIdentity:universe,lineage,events:sorted.slice(0,1)}),suffix=T.replayFromCheckpoint({checkpoint:cp,events:sorted.slice(1)}),archive=T.exportArchive({universeIdentity:universe,lineage,events});
  if(h(r.digest)!==h(suffix.digest))throw new Error('checkpoint mismatch');
  return{stateDigest:h(r.digest),eventRoot:h(T.eventRoot(events)),archiveDigest:OFU.sha256.hex(archive),order:sorted.map(e=>h(e.id))};
}
