...
const renderWitness=provider.render(ctx);assert.equal(renderWitness.authority,'MEASURED_RUNTIME_EVIDENCE');assert.ok(ops.length>10);assert.equal(renderWitness.claims.pixelOutputVisuallyInspected,false);
console.log(JSON.stringify({schema:'ofu-v2x-07-local-experience-test-1',status:'PASS',objectsCompact:a.resources,terrain:terrain.resources,frame:frame.resources,provider:provider.snapshot(),continuity,renderWitness}));
