...
const routed=[
  {name:'p1-conformance.yml',phases:'P1',minGuards:4},
  {name:'p1-p4-baseline.yml',phases:'P1,P2,P3,P4',minGuards:4},
  {name:'p2-conformance.yml',phases:'P2',minGuards:4},
  {name:'p3-conformance.yml',phases:'P3',minGuards:3},
  {name:'p4-conformance.yml',phases:'P4',minGuards:4},
  {name:'p5-conformance.yml',phases:'P5',minGuards:4},
  {name:'p5-environment-v2-canonical.yml',phases:'P5',minGuards:4},
  {name:'p6-v1-conformance.yml',phases:'P6',minGuards:4},
];
let checks=0;
...
for(const spec of routed){
  ...
  assert(/needs:\s*route\s*$/m.test(text),`${spec.name}: historical producer jobs must depend on route`);checks++;
  const routeGuards=(text.match(/needs\.route\.outputs\.run\s*==\s*'true'/g)||[]).length;
  assert(routeGuards>=spec.minGuards,`${spec.name}: heavy jobs must be guarded by route output (found ${routeGuards}, need ${spec.minGuards})`);checks++;
