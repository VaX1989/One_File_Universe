...
function createAtlas(){
  const atlas=createAtlasCore({limits:{maxEntries:32,maxRoutes:8,maxRouteStops:8,maxSerializedBytes:65536}});
  const source=atlas.saveObservation({schemaVersion:1,contract:'ofu-r6-w1-saved-observation-1',kind:'SNAPSHOT',subject,temporalRef:makeP4(100),scientificFingerprintRef:fpRef,returnRef,label:'Source'});
  return {atlas,sourceId:source.id};
}

const {atlas,sourceId}=createAtlas();
const calls=[];
...
const source=explorer.referenceFromAtlas({entryId:sourceId,domainId:'planet-history',model});
assert.equal(source.canonicalTime.seconds,'100');
assert.equal(source.p4StateRef.stateDigest,makeP4(100).stateDigest);
assert.equal(source.productContext.atlasEntryId,sourceId);
assert.equal(source.model.semanticVersion,'1.0.0');
assert.throws(()=>explorer.referenceFromAtlas({entryId:sourceId,domainId:'planet-history',model:{contractId:model.contractId,semanticVersion:'2.0.0'}}),/requested time model version mismatch/,'Atlas exact state may not be silently rebound to a different domain-time model version');
...
await explorer.materialize(explorer.planTo({reference:source,targetTime:{seconds:'200',micros:'0'}}));
await explorer.materialize(explorer.planTo({reference:source,targetTime:{seconds:'300',micros:'0'}}));
...
const instant=referenceOnly.referenceFromAtlas({entryId:sourceId,domainId:'observation-only',model:unsupportedDomain.model});
...
const resolverUnsupportedSource=resolverUnsupported.referenceFromAtlas({entryId:sourceId,domainId:'unsupported-resolver',model});
...
const mismatchSource=mismatchExplorer.referenceFromAtlas({entryId:sourceId,domainId:'mismatch',model});
...
const wrongTimeSource=wrongTimeExplorer.referenceFromAtlas({entryId:sourceId,domainId:'wrong-time',model});
