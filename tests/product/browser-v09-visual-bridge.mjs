...
try{
 ...
 await page.waitForFunction(()=>OFU?.v09ExplorerScene?.seamVersion===2&&OFU?.v09VisualUniverseBridge?.state?.ready&&OFU?.v09VisualUniverse?.snapshot().sceneStatus==='READY',{},{timeout:30000});
 const first=await page.evaluate(()=>({bridge:OFU.v09VisualUniverseBridge.snapshot(),visual:OFU.v09VisualUniverse.snapshot(),scene:OFU.v09ExplorerScene.snapshot(),canvas:!!document.getElementById('visual-universe-overlay')}));
 ...
 await page.click('[data-render-stage="approach"]');await page.waitForFunction(()=>OFU.v09VisualUniverse.snapshot().stage==='approach');
 await page.click('[data-render-stage="system"]');await page.waitForFunction(()=>OFU.v09VisualUniverse.snapshot().stage==='system');
 await page.screenshot({path:path.join(evidenceDir,'v09-converged-system.png'),fullPage:true});
 const final=await page.evaluate(()=>({bridge:OFU.v09VisualUniverseBridge.snapshot(),visual:OFU.v09VisualUniverse.snapshot()}));if(errors.length)throw new Error('page errors '+JSON.stringify(errors));
 const evidence={status:'PASS',exactSourceSha:sourceSha,sceneSeamVersion:2,visualAuthority:final.visual.authority,sceneStatus:final.visual.sceneStatus,canonicalBodies:final.visual.canonicalBodyCount,canonicalStars:final.visual.canonicalStarCount,decorativeStars:final.visual.decorativeStarCount,selectedOrbitSlot:final.visual.selectedOrbitSlot,bridgeUpdates:final.bridge.updates,legacyPollingDisabled:true,screenshot:'v09-converged-system.png'};fs.writeFileSync(path.join(evidenceDir,'v09-visual-bridge.json'),JSON.stringify(evidence,null,2)+'\n');console.log(JSON.stringify(evidence));
