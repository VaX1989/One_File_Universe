...
try{
 ...
 await page.waitForFunction(()=>__OFU_BASELINE_REPORT__?.status==='READY'&&OFU?.pxProduct?.snapshot().registry.bindingsSealed&&OFU?.v09ExplorerBeta?.state?.ready&&OFU?.productUI&&document.getElementById('beta-modeled-discovery'),null,{timeout:30000});
 await page.evaluate(()=>OFU.productUI.workspace('explore',{focus:false,announceChange:false}));
 await page.waitForFunction(()=>{const panel=document.querySelector('[data-workspace-panel="explore"]'),tab=document.querySelector('button[data-workspace="explore"]');return !!panel&&!panel.hidden&&tab?.getAttribute('aria-selected')==='true'&&OFU.productUI.state.workspace==='explore'&&document.documentElement.dataset.workspace==='explore'},null,{timeout:10000});
 await page.waitForFunction(()=>[...document.querySelectorAll('[data-beta-modeled-goal]')].every(b=>!b.disabled),null,{timeout:10000});
 const visibility=await page.evaluate(()=>{const b=document.querySelector('[data-beta-modeled-goal="ANY"]'),r=b?.getBoundingClientRect(),s=b?getComputedStyle(b):null,p=b?.closest('[data-workspace-panel]');return{workspace:OFU.productUI?.state?.workspace,panelHidden:p?.hidden??null,width:r?.width??0,height:r?.height??0,display:s?.display??null,visibility:s?.visibility??null,opacity:s?.opacity??null}});assert.equal(visibility.workspace,'explore');assert.equal(visibility.panelHidden,false);assert(visibility.width>0&&visibility.height>0&&visibility.display!=='none'&&visibility.visibility!=='hidden'&&visibility.opacity!=='0','modeled discovery must be visibly reachable '+JSON.stringify(visibility));
