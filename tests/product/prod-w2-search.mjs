...
const sparseAddressSpace={systemDiscoveryContract:'BOUNDED_SITE_WINDOW_V1',discoverNearbySystems:({limit,maxProbes,radiusSites})=>({bounded:true,systems:[sparseNode(2),sparseNode(1)].slice(0,limit),probes:2,maxProbes,radiusSites})};
...
const sparseResults=await boundedSearch({providers:[sparseProvider]});assert.deepEqual(sparseResults.results.map(x=>x.identity.canonicalId),[h(301),h(302)]);assert.ok(sparseResults.results.every(x=>x.provenance.bounded&&x.provenance.wholeUniverseEnumeration===false&&x.provenance.exactAddressReference));
