...
const rep=P.inspect('v1.representation.microscopic').value;assert.equal(rep.spatialHandoff,'human');assert.equal(rep.geometricZoomClaim,false);assert.deepEqual(rep.regimeOrder,['material','microstructure','molecular','atomic']);assert.deepEqual(Object.keys(rep.regimes).sort(),['atomic','material','microstructure','molecular']);assert.deepEqual(rep.legacy.regimeOrder,['tissue','cell','molecular','atomic']);assert.equal(rep.sourceContextReturn,'EXACT_WORLD_AND_LOCATION_AND_OBJECT');cases+=4;
const interaction=P.inspect('v1.interaction.universal-exploration').value;assert.equal(interaction.spatialScales[1],'galactic_region');assert.equal(interaction.spatialScales.at(-1),'human');assert.deepEqual(interaction.modelRegimes,['material','microstructure','molecular','atomic']);assert.deepEqual(interaction.legacyModelRegimes,['tissue','cell','molecular','atomic']);assert.equal(interaction.reverseTraversal,true);assert.equal(interaction.worldChangesRequireP4Admission,true);cases+=5;
