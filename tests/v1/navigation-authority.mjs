...
assert.equal(runtime.dispose(),true,'first Living runtime disposal must release owned authority');assert.equal(runtime.dispose(),false,'Living runtime disposal must be idempotent');assert.equal(R.snapshot().navigationAuthorityBound,false,'disposed Living runtime must not retain scale/navigation authority');cases+=3;
