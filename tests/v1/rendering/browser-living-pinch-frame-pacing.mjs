...
const sample=(refreshVisual=true)=>page.evaluate(async refreshVisual=>{
 ...
 const renderer=product.renderer.state(),pacing=product.runtime.navigationPacingSnapshot();
 ...
 let pixels;
 if(gpuPrimary){const gl=canvas.getContext('webgl2');pixels=new Uint8Array(canvas.width*canvas.height*4);gl.readPixels(0,0,canvas.width,canvas.height,gl.RGBA,gl.UNSIGNED_BYTE,pixels);if(gl.getError()!==gl.NO_ERROR)throw new Error('Deep3D pinch visual sampling failed');}
...
try{
 ...
 if(before.canvas.gpuPrimary)assert.deepEqual({surface:before.canvas.surfaceId,hidden:before.canvas.hidden},{surface:'living-gl',hidden:false},'pinch evidence must sample the visible Deep3D primary surface');
 ...
 const released=await sample(false);
