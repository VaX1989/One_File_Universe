...
assert.deepEqual(I.TECHNOLOGY_CAPABILITIES,['production','transport','materials','energy','communication','medicine','construction','conflict']);
assert.equal(I.technologyLevel(Object.fromEntries(I.TECHNOLOGY_CAPABILITIES.map(key=>[key,0]))),0);
assert.equal(I.technologyLevel(Object.fromEntries(I.TECHNOLOGY_CAPABILITIES.map(key=>[key,8]))),1000000);
const zeroWithPpm=Object.freeze({...Object.fromEntries(I.TECHNOLOGY_CAPABILITIES.map(key=>[key,0])),knowledgeContinuityPpm:1000000,exchangePpm:1000000,environmentImpactPpm:1000000});
assert.equal(I.technologyLevel(zeroWithPpm),0,'ppm metadata must not masquerade as discrete technology capability');
const twoAcross=Object.freeze({...Object.fromEntries(I.TECHNOLOGY_CAPABILITIES.map(key=>[key,2])),knowledgeContinuityPpm:0,exchangePpm:0,environmentImpactPpm:0});
assert.equal(I.technologyLevel(twoAcross),250000);
const fourAcross=Object.freeze({...Object.fromEntries(I.TECHNOLOGY_CAPABILITIES.map(key=>[key,4])),knowledgeContinuityPpm:0,exchangePpm:0,environmentImpactPpm:0});
assert.equal(I.technologyLevel(fourAcross),500000);
const lowDomains=I.knowledgeDomains({...civilization,tradeEdges:[],technology:zeroWithPpm},settlement,0);assert(!lowDomains.includes('CRAFT_TECHNIQUES'));assert(!lowDomains.includes('INSTITUTIONAL_PROCEDURES'));
const midDomains=I.knowledgeDomains({...civilization,tradeEdges:[],technology:twoAcross},settlement,0);assert(midDomains.includes('CRAFT_TECHNIQUES'));assert(!midDomains.includes('INSTITUTIONAL_PROCEDURES'));
const highDomains=I.knowledgeDomains({...civilization,tradeEdges:[],technology:fourAcross},settlement,0);assert(highDomains.includes('CRAFT_TECHNIQUES'));assert(highDomains.includes('INSTITUTIONAL_PROCEDURES'));
...
console.log(JSON.stringify({status:'PASS',suite:'v1.1 civilization representatives',version:I.VERSION,sample:a.sampleCount,identity:a.representatives[0].identityClass,authority:a.authority.class,technologyLevelPpm:I.technologyLevel(civilization.technology)}));