...
assert.deepEqual(a,b);assert.equal(a.authority.class,'MODEL_DERIVED_SIMULATION');assert.equal(a.fidelity.canonicalMeasurementsPromoted,false);assert.equal(a.fidelity.uncertaintyPpm,undefined);assert.ok(Number.isInteger(a.fidelity.heuristicUncertaintyPpm));assert.deepEqual({...a.fidelity.uncertaintySemantics,meaning:undefined},{heuristic:true,calibrated:false,empirical:false,probability:false,errorBar:false,meaning:undefined});assert.equal(a.composition.sumPpm,1000000);assert.equal(a.composition.metalPpm+a.composition.silicatePpm+a.composition.icePpm+a.composition.lightVolatilePpm,1000000);assert.equal(a.atmosphere.conserved,true);assert.equal(a.atmosphere.escapedUnits+a.atmosphere.interiorUnits+a.atmosphere.surfaceCondensedUnits+a.atmosphere.atmosphereUnits,a.atmosphere.initialInventoryUnits);
