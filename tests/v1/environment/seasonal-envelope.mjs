...
assert.equal(a.snapshots.length,4);assert.equal(a.locationIdentity,point.locationIdentity);
assert.ok(a.snapshots.every(x=>x.sourceLocationIdentity===point.locationIdentity));
assert.equal(new Set(a.snapshots.map(x=>x.sampleLocationIdentity)).size,1);assert.equal(a.sampleLocationIdentity,a.snapshots[0].sampleLocationIdentity);
assert.notEqual(a.locationIdentity,a.sampleLocationIdentity,'world-context source identity must remain distinct from detail-level environment sample identity');
assert.equal(a.locationIdentitySemantics,'WORLD_CONTEXT_SOURCE_AND_ENVIRONMENT_SAMPLE_IDENTITIES_ARE_DISTINCT');
...
for(const s of a.snapshots){assert.ok(Number.isInteger(s.localMeanTemperatureMilliK));assert.ok(s.waterActivityPpm>=0&&s.waterActivityPpm<=1000000);assert.equal(s.authority,'MODEL_DERIVED_SIMULATION');assert.equal(s.latMicroDeg,point.latMicroDeg);assert.equal(s.lonMicroDeg,point.lonMicroDeg);}
...
assert.equal(wrapped.planetIdentity,planet.planetIdentity);assert.equal(wrapped.seasonalEnvironment.locationIdentity,point.locationIdentity);assert.equal(wrapped.seasonalEnvironment.sampleCount,4);assert.equal(wrapped.committedCanonicalHistory,false);assert.equal(wrapped.canonicalP6Unchanged,true);
