...
const plan=loadComponents(),tests=loadConformance(),catalogs=providerCatalogsForConformance(plan);
...
const {O}=load(),seal=O.pxProduct.registry.snapshot();ok(seal.sealed&&seal.bindingsSealed,'actual runtime registry metadata and implementations are sealed');const providers=catalogs.flatMap(c=>c.providers),byProvider=new Map(providers.map(row=>[row.id,row])),unbound=seal.order.filter(id=>!seal.bound.includes(id)),unboundMandatory=unbound.filter(id=>byProvider.get(id)?.mandatory);ok(unboundMandatory.length===0,'every mandatory provider is bound: '+JSON.stringify(unboundMandatory));ok(JSON.stringify(unbound)===JSON.stringify(['v2x13.render.webgl2-resources']),'only the explicitly optional direct V2X13 pixel consumer may remain outside PX invocation: '+JSON.stringify(unbound));ok(ids.includes('v2x13.render.webgl2-resources'),'optional V2X13 provider must retain an executable component');
...
ok(product.px.scope==='FULL_V2_LIVING_PRODUCT_OVER_V1_COMPATIBILITY_RUNTIME','production scope is the actual V2 Living composition');
