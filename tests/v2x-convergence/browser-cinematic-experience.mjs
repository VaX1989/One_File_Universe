...
async function boot(viewport,{reducedMotion='no-preference',name='desktop'}={}){
 ...
 async function frame(label){
  ...
  if(['UNIVERSE','GALAXY','REGION','NEIGHBORHOOD','SYSTEM'].includes(state.stage)){assert(state.render.cinematicMacro?.active,'macro stage must use cinematic macro director');assert.equal(state.render.cinematicMacro.authority,'PRESENTATION_ONLY');assert.equal(state.render.cinematicMacro.semanticMutation,false);assert.equal(state.render.cinematicMacro.cameraAuthority,false);assert.equal(state.render.cinematicMacro.networkResources,0);assert(state.render.cinematicMacro.hitCount>0,'macro presentation must retain direct manipulation targets');}
...
try{
 ...
 const canvasBox=await desktop.page.locator('#living-view').boundingBox();assert(canvasBox,'Living viewport must have a real pointer surface');await desktop.page.mouse.move(canvasBox.x+canvasBox.width*.86,canvasBox.y+canvasBox.height*.76);await desktop.page.mouse.down();await desktop.page.mouse.move(canvasBox.x+canvasBox.width*.84,canvasBox.y+canvasBox.height*.75,{steps:2});await desktop.page.mouse.up();await desktop.page.waitForFunction(()=>document.getElementById('living-stage').dataset.cinematicGuidance==='DISMISSED');assert.equal(await desktop.page.locator('#v2-cinematic-guide').isHidden(),true,'first-run guidance must yield immediately to real user input');
