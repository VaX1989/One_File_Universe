...
try{
 ...
 const body=await desktop.page.evaluate(()=>{const r=OFU.v1LivingProduct.runtime,s=r.snapshot(),row=s.rows.find(x=>x.kind==='planet')||s.rows.find(x=>x.kind==='moon');if(!row)throw new Error('system has no modeled world for cinematic witness');r.enterBody(row);return row.canonicalId||row.entityId;});await desktop.waitStage('ORBIT');await desktop.page.evaluate(()=>OFU.v1LivingProduct.runtime.approach());await desktop.waitStage('APPROACH');s=await desktop.frame('03-approach');assert.equal(s.regime,'PLANETARY');assert(s.render.gpu===null||s.render.gpu.authority==='PRESENTATION_ONLY');
 await desktop.page.evaluate(()=>OFU.v1LivingProduct.runtime.at(0,0,{stage:'REGIONAL_SURFACE'}));await desktop.waitStage('REGIONAL_SURFACE');s=await desktop.frame('04-surface');assert.equal(s.regime,'SURFACE');
 await desktop.page.evaluate(()=>OFU.v1LivingProduct.runtime.scale('HUMAN'));await desktop.waitStage('HUMAN');s=await desktop.frame('05-human');assert.equal(s.regime,'LOCAL');
 const enteredMicro=await desktop.page.evaluate(()=>{const r=OFU.v1LivingProduct.runtime,s=r.snapshot(),row=s.rows.find(x=>!['SETTLEMENT','RUIN'].includes(x.kind));if(!row)return false;r.selectObject(row.entityId);r.enterMicro();return true;});if(enteredMicro){await desktop.waitStage('MATERIAL');s=await desktop.frame('06-matter');assert.equal(s.regime,'MICRO');await desktop.page.evaluate(()=>OFU.v1LivingProduct.runtime.deeper());const microStage=await desktop.page.evaluate(()=>OFU.v1LivingProduct.runtime.snapshot().stage);await desktop.waitStage(microStage);s=await desktop.frame('07-micro');assert.equal(s.regime,'MICRO');}
