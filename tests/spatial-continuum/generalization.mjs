...
try{
  for(const [orbitSlot,latMicroDeg,lonMicroDeg] of [[0,0,0],[2,12345678,23456789],[5,-33000000,151000000]]){
    ...
    try{
      ...
      let alternate=null;
      if(orbitSlot===2){
        alternate=await page.evaluate(primary=>{const api=__OFU_SPATIAL_CONTINUUM__,targets=api.snapshot().render.pickTargets.bodies;for(const id of api.worldSet.supportedBodyIds){const metric=targets[id];if(id===primary||!metric?.visible)continue;const hit=api.renderer.pick(metric.clientX,metric.clientY),materialized=api.worldSet.get(id);if(hit?.id===id)return{id,metric,radiusM:materialized.physicalRadiusM,surfaceId:materialized.surfaceId,sampleKind:materialized.sample.kind}}return null},initial.bodyId);
        assert.ok(alternate,'at least one supported sibling world must be independently pickable');
        ...
        const selected=await page.evaluate(()=>({snapshot:__OFU_SPATIAL_CONTINUUM__.snapshot(),bodyId:__OFU_SPATIAL_CONTINUUM__.world.bodyId,surfaceId:__OFU_SPATIAL_CONTINUUM__.world.surfaceId}));assert.equal(selected.bodyId,alternate.id);assert.equal(selected.surfaceId,alternate.surfaceId);assert.equal(selected.snapshot.state.graph.focusId,alternate.id);assert.equal(selected.snapshot.state.camera.derivation.fromTargetId,alternate.id);assert.equal(selected.snapshot.state.camera.derivation.fromRadiusM,alternate.radiusM);assert.equal(selected.snapshot.render.pickTargets.body.visible,true);assert.equal(selected.snapshot.render.sceneCount,1);assert.equal(selected.snapshot.render.cameraCount,1);await page.screenshot({path:path.join(evidenceDir,'world-alternate-orbit.png')});
        for(const stage of ['APPROACH','GLOBAL_SURFACE','REGIONAL_SURFACE','LOCAL_SURFACE','HUMAN']){await page.evaluate(stage=>{__OFU_SPATIAL_CONTINUUM__.travelTo(stage);__OFU_SPATIAL_CONTINUUM__.settle()},stage);await page.waitForTimeout(35)}const siblingHuman=await page.evaluate(()=>__OFU_SPATIAL_CONTINUUM__.snapshot());assert.equal(siblingHuman.worldIdentity,alternate.id);assert.ok(siblingHuman.state.graph.focusAncestry.includes(alternate.id));assert.ok(siblingHuman.render.activeMeshes>0);await page.screenshot({path:path.join(evidenceDir,'world-alternate-human.png')});for(const stage of ['LOCAL_SURFACE','REGIONAL_SURFACE','GLOBAL_SURFACE','APPROACH','ORBIT','SYSTEM']){await page.evaluate(stage=>{__OFU_SPATIAL_CONTINUUM__.travelTo(stage);__OFU_SPATIAL_CONTINUUM__.settle()},stage);await page.waitForTimeout(25)}
        await page.selectOption('#continuum-body-select',initial.bodyId);await page.waitForTimeout(80);const shape=state=>({meshes:state.render.meshCount,materials:state.render.materialCount,textures:state.render.textureCount,cameras:state.render.cameraCount,scenes:state.render.sceneCount}),initialShape=shape(await page.evaluate(()=>__OFU_SPATIAL_CONTINUUM__.snapshot()));let siblingShape=null;for(let cycle=0;cycle<3;cycle++){await page.selectOption('#continuum-body-select',alternate.id);await page.waitForTimeout(80);const nextShape=shape(await page.evaluate(()=>__OFU_SPATIAL_CONTINUUM__.snapshot()));if(siblingShape)assert.deepEqual(nextShape,siblingShape,'sibling rebind resources must be stable');else siblingShape=nextShape;await page.selectOption('#continuum-body-select',initial.bodyId);await page.waitForTimeout(80);assert.deepEqual(shape(await page.evaluate(()=>__OFU_SPATIAL_CONTINUUM__.snapshot())),initialShape,'return rebind resources must be stable')}
      }
      ...
      results.push({orbitSlot,bodyId:initial.bodyId,bodyClass:initial.bodyClass,radiusM:initial.radiusM,surfaceId:initial.surfaceTarget.locationIdentity,sampleKind:initial.sampleKind,sampleId:initial.sampleId,latitudeDeg:initial.surfaceTarget.latitudeDeg,longitudeDeg:initial.surfaceTarget.longitudeDeg,alternatePick:alternate?{id:alternate.id,radiusM:alternate.radiusM,surfaceId:alternate.surfaceId,sampleKind:alternate.sampleKind,materializedInPlace:true}:null,resources:{meshes:reversed.render.meshCount,materials:reversed.render.materialCount,textures:reversed.render.textureCount,totalVertices:reversed.render.totalVertices,terrainPatchPool:reversed.render.terrainPatchPool},rendererProfile:reversed.render.profile});
  ...
  const output={status:'PASS',suite:'spatial-continuum-r3-generalization',worlds:results.length,fullForwardReverse:true,rendererOwnedAlternateBodyPicking:true,dynamicSiblingMaterialization:true,singleSceneRebinding:true,targetDerivedCamera:true,frameDerivedRendering:true,surfaceTargetContinuity:true,results};
