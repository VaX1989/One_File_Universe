...
const materializer=createProgressiveMacroMaterializer({maxResident:4,maxBuildPerSlice:2,now:()=>clock});
...
// Repeated movement reuses overlap and caps duplicate transition residency.
...
// Rapid reverse supersedes stale work atomically and preserves the previous committed context.
materializer.begin(make(['c','d','e','f']));
step=materializer.runSlice({build:item=>Object.freeze({...item,built:true}),budget:1});
assert.equal(step.status,'COMMITTED'); // three reused + one build complete the target.
...
// Long traversal remains bounded and explicit disposal returns to zero residency.
...
let disposed=0;
const disposal=materializer.dispose({disposeValue:()=>disposed++,reason:'TEST_END'});
...
console.log(JSON.stringify({
  contract:'ofu-r6-w0-progressive-macro-materialization-test-1',
  status:'PASS',
  coldSlices:2,
