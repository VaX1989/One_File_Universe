...
assert.match(human,/createSurfaceTerrainSampler/);assert.match(human,/rings=\[180,420,860,1600\]/);assert.match(human,/PRESENTATION_ONLY/);assert.match(human,/physical=encoded\/(mesh\.metadata\.sample\?1\.35:1\)/);assert.match(human,/desired=physical\*\(mesh\.metadata\.sample\?1\.12:1\)/);assert.match(human,/depthBandPolicy:'DETAIL_HAZE_PRIORITY_NOT_WORLD_SCALE'/);
