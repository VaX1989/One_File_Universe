...
try{
 ...
 if(unsupportedUi.reason!=='P5_MASS_DOMAIN'||unsupportedUi.environment!=='Not evaluated — target unsupported'||unsupportedUi.biology!=='Not evaluated — target unsupported'||unsupportedUi.environmentProjection!==null||unsupportedUi.p6Projection!==null||unsupportedUi.targetState!=='UNSUPPORTED'||unsupportedUi.viewportHidden!==false||!/unsupported/i.test(unsupportedUi.viewportStatus||'')||!/P5[_ ]MASS[_ ]DOMAIN/i.test(unsupportedUi.viewportStatus||'')||unsupportedUi.persistentStatusAccessible!==true||unsupportedUi.testOnly)throw new Error('real MASS_DOMAIN unsupported product truthfulness failed '+JSON.stringify(unsupportedUi));
