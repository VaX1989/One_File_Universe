...
try{
 ...
 await page.waitForFunction(()=>__OFU_PLANET_PREVIEW__?.targetStatus==='UNSUPPORTED'&&__OFU_PLANET_PREVIEW__?.targetReason==='MASS_DOMAIN',undefined,{timeout:20000});
 await page.evaluate(()=>OFU.productUI.workspace('explore',{announceChange:false}));
 ...
 if(unsupportedUi.reason!=='MASS_DOMAIN'||unsupportedUi.environment!=='Not evaluated — target unsupported'||unsupportedUi.biology!=='Not evaluated — target unsupported'||unsupportedUi.environmentProjection!==null||unsupportedUi.p6Projection!==null||unsupportedUi.targetState!=='UNSUPPORTED'||unsupportedUi.viewportHidden!==false||!/unsupported/i.test(unsupportedUi.viewportStatus||'')||!/mass domain/i.test(unsupportedUi.viewportStatus||'')||unsupportedUi.persistentStatusAccessible!==true||unsupportedUi.testOnly)throw new Error('real MASS_DOMAIN unsupported product truthfulness failed '+JSON.stringify(unsupportedUi));
