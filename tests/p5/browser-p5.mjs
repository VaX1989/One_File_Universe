...
const build=JSON.parse(fs.readFileSync('dist/p5-build-manifest.json','utf8')),golden=JSON.parse(fs.readFileSync('tests/vectors/golden-p5-corpus-v1.json','utf8')),file=path.resolve('dist/One_File_Universe.html'),url=pathToFileURL(file).href;
...
try{
  ...
  const expected=golden.shippedCrossRuntimeVector;if(result.planetId!==expected.planetId||result.physicalDigest!==expected.physicalDigest||result.terrainDigest!==expected.terrainDigest)throw new Error('Golden shipped cross-runtime vector drift: '+JSON.stringify({expected,result}));
  ...
  const evidence={status:'PASS',test:'P5-SHIPPED-CANDIDATE-v1',sourceCommit:process.env.OFU_SOURCE_SHA,goldenCorpusVersion:golden.corpusVersion,browser:name,browserVersion:browser.version(),playwrightVersion,nodeVersion:process.version,platform:process.platform,arch:process.arch,osRelease:os.release(),artifactSha256:build.artifactSha256,artifactBytes:build.artifactBytes,componentManifestHash:build.componentManifestHash,...result,unexpectedNetworkRequests:unexpected.length,realSafariIOSVerified:false};
