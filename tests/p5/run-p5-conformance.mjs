...
assert(maxRelativeError<0.00001,'bounded fixed rocky radius exceeds certified approximation error');
...
const digest=P.hex(P5.physicalDigest(first));assert.match(digest,/^[0-9a-f]{64}$/);
console.log(JSON.stringify({status:'PASS',contract:P5.PHYSICAL_CONTRACT,model:P5.VERSION,planetId:P.hex(first.planetId),goldenCandidateDigest:digest,baselineMassMilliEarth:String(snapshot.formation.baselineMassMilliEarth),coreMassFractionPermille:String(first.physical.composition.coreMassFractionPermille),radiusM:String(first.physical.meanRadiusM),gravityMicroMs2:String(first.physical.surfaceGravityMicroMs2),densityKgM3:String(first.physical.meanDensityKgM3),numeric:{maxRelativeError},terrain:{patchVertices:25,globalHeightmap:false,faceAuditThroughLevel:3,refinementOrderIndependent:true,reconcileMaterializedVertices:125},p6Projection:env.status},null,2));
