...
loadProduct();
const R=OFU.waveIVScaleRuntime;assert.equal(R.VERSION,'ofu-wave-iv-scale-runtime-3');assert.deepEqual([...R.LADDER],['galaxy','galactic_region','stellar_neighborhood','system','orbit','approach','global_surface','regional_surface','local_surface','human']);assert.ok(R.snapshot().semanticScale);assert.equal(R.normalizeBand('close'),'human');assert.equal(R.normalizeBand('neighborhood'),'stellar_neighborhood');
const activations=[];for(const [id,scales] of [['wave-iv-macro',['galaxy','galactic_region','stellar_neighborhood','system']],['planet-webgl',['orbit','approach','global_surface']],['surface-webgl',['regional_surface','local_surface','human']]])R.registerSceneProvider({id,scales,setActive(active,s){if(active)activations.push([id,s.semanticScale])}});
for(const scale of R.LADDER){R.requestStage(scale,{source:'oracle',driveCamera:false});assert.equal(R.snapshot().semanticScale,scale);const expected=['galaxy','galactic_region','stellar_neighborhood','system'].includes(scale)?'wave-iv-macro':['orbit','approach','global_surface'].includes(scale)?'planet-webgl':'surface-webgl';assert.equal(R.snapshot().activeSceneProvider,expected)}
const key={galaxyX:48n,galaxyY:-50n,galaxyZ:-1n,sectorX:0n,sectorY:0n,sectorZ:0n,siteX:61n,siteY:0n,siteZ:0n,orbitSlot:0n};R.setSelection(key,{planetId:OFU.pxProduct.captured(key).selection.target.entityId,presentationStatus:'SUPPORTED'});const before=R.snapshot().selectedCanonicalTarget;R.requestStage('galaxy',{driveCamera:false});R.requestStage('human',{driveCamera:false});assert.deepEqual(R.snapshot().selectedCanonicalTarget,before);
