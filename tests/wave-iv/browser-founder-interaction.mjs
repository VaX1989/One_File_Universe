...
await stage('global_surface');await page.waitForFunction(()=>__OFU_PLANET_PREVIEW__.snapshot().surfaceMode==='GLOBE');await page.waitForTimeout(300);const globe=await page.evaluate(()=>{const preview=globalThis.__OFU_PLANET_PREVIEW__.snapshot(),c=document.getElementById('planet-view'),gl=c.getContext('webgl2');return{backend:preview.backend||'none',glAvailable:!!gl,rendererVersion:OFU.planetWebGL2?.VERSION||null,surfaceMode:preview.surfaceMode,canvasWidth:c.width,canvasHeight:c.height}});assert.equal(globe.backend,'webgl2');assert.equal(globe.glAvailable,true);assert.equal(globe.rendererVersion,'ofu-planet-webgl2-presentation-4');assert.equal(globe.surfaceMode,'GLOBE');assert(globe.canvasWidth>0&&globe.canvasHeight>0);

await page.setViewportSize({width:320,height:700});await page.waitForTimeout(150);const responsive=await page.evaluate(()=>{const p=document.getElementById('planet-view').getBoundingClientRect(),rail=document.getElementById('wave-iv-scale-rail').getBoundingClientRect();return{innerWidth,scrollWidth:document.documentElement.scrollWidth,planet:{left:p.left,right:p.right,width:p.width},rail:{left:rail.left,right:rail.right,width:rail.width},touch:getComputedStyle(document.getElementById('planet-view')).touchAction}});assert(responsive.scrollWidth<=responsive.innerWidth+1,'mobile layout must not overflow horizontally');assert(responsive.planet.left>=-1&&responsive.planet.right<=responsive.innerWidth+1,'planet canvas must fit mobile viewport');assert(responsive.rail.left>=-1&&responsive.rail.right<=responsive.innerWidth+1,'scale rail must fit mobile viewport');assert.equal(responsive.touch,'none');
