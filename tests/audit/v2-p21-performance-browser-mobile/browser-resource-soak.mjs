...
async function openOfflineProduct(page,browserName){
  ...
  try{
   ...
   try{
    startupState=await page.evaluate(()=>{
     const probe=document.createElement('canvas');let gl=null,probeError=null,renderer=null,vendor=null;
     try{
      gl=probe.getContext('webgl2',{alpha:true,antialias:false,premultipliedAlpha:false,preserveDrawingBuffer:false,depth:true});
      if(gl){const debug=gl.getExtension('WEBGL_debug_renderer_info');renderer=debug?gl.getParameter(debug.UNMASKED_RENDERER_WEBGL):gl.getParameter(gl.RENDERER);vendor=debug?gl.getParameter(debug.UNMASKED_VENDOR_WEBGL):gl.getParameter(gl.VENDOR);}
     }catch(error){probeError=String(error?.message||error)}
     const webgl2Available=!!gl;
     try{gl?.getExtension?.('WEBGL_lose_context')?.loseContext?.()}catch{}
     return {
     readyState:document.readyState,
     userAgent:navigator.userAgent,
     hardwareConcurrency:navigator.hardwareConcurrency,
     webgl2Probe:{available:webgl2Available,error:probeError,renderer,vendor},
     canvases:[...document.querySelectorAll('canvas')].map(canvas=>({id:canvas.id||null,width:canvas.width,height:canvas.height,connected:canvas.isConnected})),
   ...
   const diagnostic=`browser=${browserName} startupConsole=${JSON.stringify(startupConsole)} startupState=${JSON.stringify(startupState)}`;
...
async function framePacing(page,{targetSamples=90,deadlineMs=12000}={}){
 ...
 const result=await page.evaluate(({targetSamples,deadlineMs})=>new Promise(resolve=>{
  const samples=[];let last=performance.now(),left=targetSamples,settled=false,frame=0;
  const finish=completed=>{if(settled)return;settled=true;clearTimeout(timer);if(frame)cancelAnimationFrame(frame);resolve({completed,samples})};
  const timer=setTimeout(()=>finish(false),deadlineMs);
  const tick=now=>{samples.push(now-last);last=now;if(--left<=0)finish(true);else frame=requestAnimationFrame(tick)};
  frame=requestAnimationFrame(tick);
 }),{targetSamples,deadlineMs});
 const samples=result.samples||[],sorted=[...samples].sort((a,b)=>a-b),pick=q=>sorted[Math.min(sorted.length-1,Math.floor(sorted.length*q))]||0;
...
function sustainedGrowth(values){if(values.length<4)return false;const deltas=values.slice(1).map((value,index)=>value-values[index]),increases=deltas.filter(value=>value>0).length;return deltas.every(value=>value>=0)&&increases>=Math.ceil(deltas.length/2)&&values.at(-1)>values[0]}
...
async function pointerClick(page,locator){
 const start=Date.now();await locator.waitFor({state:'visible',timeout:15000});if(await page.evaluate(()=>/Firefox\//.test(navigator.userAgent))){await locator.dispatchEvent('click');return}
...
async function desktopSoak(browserName,browser){
 ...
 const warm=samples.slice(Math.floor(samples.length/3));const fields=['domNodes','canvasCount','listeners','workers'];const plateauEvidence={};for(const f of fields){const vals=warm.map(x=>x[f]).filter(Number.isFinite);plateauEvidence[f]={values:vals,slope:slope(vals),sustainedGrowth:sustainedGrowth(vals),plateau:plateau(vals,f==='listeners'?8:f==='domNodes'?16:2)};assert.equal(plateauEvidence[f].plateau,true,`${browserName} ${f} failed bounded plateau ${JSON.stringify(vals)}`);assert.equal(plateauEvidence[f].sustainedGrowth,false,`${browserName} ${f} showed sustained monotonic growth ${JSON.stringify(vals)}`)}
 const heap=warm.map(x=>x.heap).filter(Number.isFinite);let heapEvidence={status:'NOT_MEASURABLE'};if(heap.length){const first=heap[0],last=heap.at(-1),limit=Math.max(first*2,first+64*1024*1024);heapEvidence={status:'MEASURED_BROWSER_JS_HEAP_ONLY',first,last,max:Math.max(...heap),slope:slope(heap),sustainedGrowth:sustainedGrowth(heap),limit};assert.ok(last<=limit,`${browserName} JS heap exceeded conservative soak bound`)}
 ...
 progress('FRAME_PACING_START');const pacing=await framePacing(page);progress('FRAME_PACING_COMPLETE');
 ...
 const evidence={browser:browserName,controlInputMethod:browserName==='firefox'?'DOM_CLICK_EVENT_DISPATCH_FIREFOX_DRIVER_ACKNOWLEDGMENT_LIMITATION':'PLAYWRIGHT_NATIVE_POINTER',cycles:CYCLES,universeMicroUniverseRoundTrips:CYCLES,startupMs,transitionMs,transitionP95:[...transitionMs].sort((a,b)=>a-b)[Math.min(transitionMs.length-1,Math.floor(transitionMs.length*.95))],framePacing:pacing,plateau:plateauEvidence,heap:heapEvidence,longTasks:longTask,contextLoss,audio:{status:audioBounds.length?'MEASURED_RUNTIME_SNAPSHOT':'NOT_MEASURABLE_RUNTIME_SNAPSHOT_UNAVAILABLE',samples:audioBounds},offline:true,externalRequests,samples};progress('DESKTOP_CONTEXT_CLOSE_START');await context.close();progress('DESKTOP_COMPLETE');return evidence;
}

async function mobileAndA11y(browserName,browser){
 const progress=phase=>console.error(JSON.stringify({audit:'P21',browser:browserName,phase,at:new Date().toISOString()}));progress('MOBILE_BOOT_START');
 const context=await browser.newContext({viewport:{width:390,height:844},deviceScaleFactor:2,isMobile:true,hasTouch:true});if(browserName!=='firefox')await instrument(context);const page=await context.newPage(),workerAudit=trackWorkers(page);const errors=[],externalRequests=[];page.on('pageerror',e=>errors.push(String(e?.message||e).slice(0,800)));page.on('request',r=>{if(/^https?:/i.test(r.url()))externalRequests.push(r.url())});await openOfflineProduct(page,browserName);await ready(page);await page.waitForFunction(()=>document.documentElement.dataset.ofuMobile==='true',{timeout:10000});progress('MOBILE_READY');
 await clickScale(page,'ORBIT');progress('MOBILE_ORBIT');const before=await page.evaluate(()=>{const product=OFU.v1LivingProduct,runtime=product.runtime.snapshot(),pacing=product.runtime.navigationPacingSnapshot(),renderer=product.renderer.state(),canvas=document.getElementById('living-view'),rect=canvas?.getBoundingClientRect(),style=canvas&&getComputedStyle(canvas);if(!canvas||!rect||rect.width<=0||rect.height<=0||style.display==='none'||style.visibility==='hidden')throw new Error('active Living viewport is not visible');return{stage:runtime.stage,semanticScale:runtime.semanticScale,coordinate:runtime.navigationCoordinate,inputEvents:pacing.inputEvents,renderFrames:renderer.metrics.frames}});
 ...
 assert.ok(range(resize.map(x=>x.dom).filter(Number.isFinite))<=16,`${browserName} mobile resize DOM growth exceeded bounded range ${JSON.stringify(resize.map(({w,h,dom,tagCounts,living})=>({w,h,dom,tagCounts,living})))}`);assert.ok(range(resize.map(x=>x.listeners).filter(Number.isFinite))<=8,`${browserName} mobile resize listener growth exceeded bounded range`);assert.ok(range(resize.map(x=>x.workers).filter(Number.isFinite))<=2,`${browserName} mobile resize worker growth exceeded bounded range`);
 progress('MOBILE_PINCH_AND_RESIZE_COMPLETE');assert.equal(errors.length,0,`${browserName} mobile errors: ${errors.join('\n')}`);assert.equal(externalRequests.length,0,`${browserName} mobile unexpected external requests: ${externalRequests.join('\n')}`);await context.close();progress('MOBILE_CONTEXT_CLOSED');
 progress('REDUCED_MOTION_BOOT_START');const reduced=await browser.newContext({viewport:{width:1024,height:768},reducedMotion:'reduce'});const rp=await reduced.newPage(),reducedExternal=[];rp.on('request',r=>{if(/^https?:/i.test(r.url()))reducedExternal.push(r.url())});await openOfflineProduct(rp,browserName);await ready(rp);progress('REDUCED_MOTION_READY');const reducedMotion=await rp.evaluate(()=>({media:matchMedia('(prefers-reduced-motion: reduce)').matches,snapshot:globalThis.__OFU_V1X10_ACCESSIBILITY__?.snapshot?.().reducedMotion??null}));assert.equal(reducedMotion.media,true);if(reducedMotion.snapshot!=null)assert.equal(reducedMotion.snapshot,true);assert.equal(reducedExternal.length,0,`${browserName} reduced-motion unexpected external requests: ${reducedExternal.join('\n')}`);await reduced.close();progress('MOBILE_COMPLETE');return{method:'BROWSER_POINTER_EVENT_EMULATION',physicalDeviceVerified:false,pinch:true,resizeOrientation:resize,reducedMotion,offline:true};
}
