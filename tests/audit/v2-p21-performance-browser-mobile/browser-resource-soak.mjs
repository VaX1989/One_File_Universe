...
async function mobileAndA11y(browserName,browser){
 ...
 await page.evaluate(()=>{const R=globalThis.OFU?.waveIVInputRouter;if(!R||typeof R.routeStage!=='function'||R.routeStage('orbit','p21-mobile-audit')!==true)throw new Error('canonical Orbit route rejected')});await page.waitForFunction(()=>OFU.waveIVScaleRuntime.snapshot().semanticScale==='orbit');const before=await page.evaluate(()=>({stage:OFU.v1LivingProduct.runtime.snapshot().stage,pinch:OFU.waveIVInputRouter.state.pinchIntents}));
