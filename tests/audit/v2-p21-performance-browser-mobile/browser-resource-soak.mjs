...
async function mobileAndA11y(browserName,browser){
 ...
 const sizes=[{width:844,height:390},{width:390,height:844},{width:700,height:320},{width:320,height:700},{width:390,height:844}];const resize=[];for(const size of sizes){await page.setViewportSize(size);await page.waitForTimeout(60);const browserState=await page.evaluate(()=>{const listenerState=globalThis.__OFU_P21_AUDIT__?.listenerSnapshot?.()||null,tagCounts={};for(const node of document.querySelectorAll('*'))tagCounts[node.tagName]=(tagCounts[node.tagName]||0)+1;return{w:innerWidth,h:innerHeight,stage:OFU.v1LivingProduct.runtime.snapshot().stage,dom:document.querySelectorAll('*').length,tagCounts,listeners:listenerState?.connectedOrGlobal??null,listenerState,overflow:document.documentElement.scrollWidth>document.documentElement.clientWidth+1}});resize.push({...browserState,workers:workerAudit.live,createdWorkers:workerAudit.created,closedWorkers:workerAudit.closed});const s=resize.at(-1);assert.equal(s.stage,pinch.stage);assert.equal(s.overflow,false)}
 assert.ok(range(resize.map(x=>x.dom).filter(Number.isFinite))<=16,`${browserName} mobile resize DOM growth exceeded bounded range ${JSON.stringify(resize.map(({w,h,dom,tagCounts})=>({w,h,dom,tagCounts})))}`);assert.ok(range(resize.map(x=>x.listeners).filter(Number.isFinite))<=8,`${browserName} mobile resize listener growth exceeded bounded range`);assert.ok(range(resize.map(x=>x.workers).filter(Number.isFinite))<=2,`${browserName} mobile resize worker growth exceeded bounded range`);
